...
// historical desync where fixtures hard-coded `gitmap-v24` next to
...
func TestLineContainsAny(t *testing.T) {
	needles := [][]byte{[]byte("gitmap-v24"), []byte("gitmap/v9")}
	if !lineContainsAny([]byte("import gitmap-v24/foo"), needles) {
...
func TestScanAuditFileFormatting(t *testing.T) {
	...
	want2 := fmt.Sprintf(constants.MsgReplaceAuditMatch, path, 2, "see gitmap-v24 for details")
