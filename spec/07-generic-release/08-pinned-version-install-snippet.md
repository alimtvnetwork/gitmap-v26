...
> **Status:** Active since gitmap-v24 **v3.12.0** (2026-04-20).
...
`gitmap-v24/release/installsnippet.go::AppendPinnedInstallSnippet` runs inside
...
```powershell
$ver = 'v3.11.1'
$installer = irm https://raw.githubusercontent.com/alimtvnetwork/gitmap-v24/main/gitmap-v24/scripts/install.ps1
...
```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v24/main/gitmap-v24/scripts/install.sh \
...
* `gitmap-v24/scripts/install.ps1` lines around the `INSTALLER_DELEGATED`
  branch — new `elseif (-not [string]::IsNullOrWhiteSpace($Version))`
  arm prints `[discovery] -Version <tag> pinned; skipping repo probe`.
* `gitmap-v24/scripts/install.sh` lines around the same branch — new
...
1. **Bump version** in `gitmap-v24/constants/constants.go` and
...
4. **Run `gitmap-v24 release`**. The publisher auto-appends the pinned
   snippet — do not paste it manually.
5. **Verify on the release page** that the snippet renders the correct
   tag (e.g. `v3.12.0`) and points at `gitmap-v24` (not `gitmap-v24`).
...
| Version | Change |
|---|---|
| v3.12.0 | Initial spec + implementation. Snippet auto-appended; both installers honor `--version` / `-Version` to skip discovery + latest lookup. Repo renamed `gitmap-v24` → `gitmap-v24` everywhere. |
