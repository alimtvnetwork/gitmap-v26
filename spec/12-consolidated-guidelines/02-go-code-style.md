...
|-------------------|------------------|----------|
| CLI routing / top-level commands | `gitmap-v24/constants/` when shared across dispatcher, completion, tests | `CmdScan`, `CmdReleaseAlias` |
...
1. Use `gitmap-v24/constants/` only for literals shared across packages.
...
`.github/scripts/check-constants-naming.sh` enforces this at a lightweight level by rejecting newly added `gitmap-v24/constants/` names that are not ordinary PascalCase identifiers.
