...
1. `gitmap-v22 commit-in <s> a b c`, `gitmap-v22 commit-in <s> a,b,c`,
   `gitmap-v22 commit-in <s> "a, b, c"`, and
   `gitmap-v22 commit-in <s> "a"  "b"   "c"` produce identical
...
4. `all` from source basename `gitmap-v22` expands to `gitmap-v22, gitmap-v22,
   gitmap-v22, …, gitmap-v22` (ascending; plain `gitmap-v22` first as `v0`).
...
19. Every enum in §02–§06 lives in its own Go file under
    `gitmap-v22/cmd/commitin/<enumname>.go`, used by both production code
...
`gitmap-v22/cmd/commitin/integration_test.go` that build a synthetic input
