...
1. `gitmap-v24 commit-in <s> a b c`, `gitmap-v24 commit-in <s> a,b,c`,
   `gitmap-v24 commit-in <s> "a, b, c"`, and
   `gitmap-v24 commit-in <s> "a"  "b"   "c"` produce identical
...
4. `all` from source basename `gitmap-v24` expands to `gitmap-v24, gitmap-v24,
   gitmap-v24, …, gitmap-v24` (ascending; plain `gitmap-v24` first as `v0`).
...
19. Every enum in §02–§06 lives in its own Go file under
    `gitmap-v24/cmd/commitin/<enumname>.go`, used by both production code
...
`gitmap-v24/cmd/commitin/integration_test.go` that build a synthetic input
