...
branches are interchangeable with `gitmap-v25 release-branch` and friends.
...
This is exactly the regex used by `gitmap-v25 release` for release-branch
...
- **`<200` lines per file**: the new package
  `gitmap-v25/cmd/commitin/replay/tags.go` holds the orchestration only;
  git plumbing lives in `gitmap-v25/gitutil/tags.go` (annotated-tag
  read/write helpers).
