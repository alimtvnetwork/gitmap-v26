...
branches are interchangeable with `gitmap-v24 release-branch` and friends.
...
This is exactly the regex used by `gitmap-v24 release` for release-branch
...
- **`<200` lines per file**: the new package
  `gitmap-v24/cmd/commitin/replay/tags.go` holds the orchestration only;
  git plumbing lives in `gitmap-v24/gitutil/tags.go` (annotated-tag
  read/write helpers).
