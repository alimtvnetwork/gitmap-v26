...
gitmap-v25 binary location, and integrates Linux/macOS shell-based update
...
```
  ...
  Binary:    /home/alim/.local/bin/gitmap-v25
...
The `gitmap-v25 update` command's `executeUpdate` function only supported
...
1. Resolves the active binary's install directory using `resolveInstalledDir()`:
   - First tries `exec.LookPath("gitmap-v25")` to find the PATH binary.
...
3. `run.sh` already handles:
   ...
   - **PATH binary sync** (lines 601-618): if the active `which gitmap-v25`
...
|----------|--------|-------------|
| 1 | `exec.LookPath("gitmap-v25")` | Finds the binary on PATH |
...
| File | Change |
|------|--------|
| `gitmap-v25/cmd/installeddir.go` | New: `runInstalledDir()` command |
| `gitmap-v25/cmd/updatescript.go` | Added `executeUpdateUnix()`, `resolveInstalledDir()` |
| `gitmap-v25/cmd/rootutility.go` | Added `installed-dir` / `id` dispatch |
| `gitmap-v25/constants/constants_cli.go` | Added `CmdInstalledDir`, `CmdInstalledDirAlias`, `HelpInstalledDir` |
| `gitmap-v25/constants/constants_update.go` | Added `ErrUpdateNoRunSH`, `MsgUpdateInstallDir` |
| `gitmap-v25/helptext/installed-dir.md` | New: command documentation |

---

## 5. Acceptance Criteria

1. `gitmap-v25 installed-dir` prints the binary path and directory.
2. `gitmap-v25 id` produces the same output.
3. `gitmap-v25 update` on Linux/macOS runs `run.sh --update` from the source repo.
4. After update, `gitmap-v25 version` reflects the latest version.
