...
The compress/checksum step ran inside `gitmap-updater/` (which has no `dist/` folder) instead of `gitmap-v22/dist/` where cross-compiled binaries are output. The `cd dist` command assumed the working directory was `gitmap-v22/`, but GitHub Actions defaults to the repository root.
