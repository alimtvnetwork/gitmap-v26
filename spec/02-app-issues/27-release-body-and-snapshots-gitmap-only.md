# `gitmap-v24 release` must not leak gitmap-specific content into other repos' releases

## Symptom

Running `gitmap-v24 release` inside a non-gitmap repository (e.g. `img-pdf-v2`,
...
1. **Attached `release-version-vX.Y.Z.ps1` / `release-version-vX.Y.Z.sh`
   ...
   `REPO="alimtvnetwork/gitmap-v24"` and `BINARY_NAME="gitmap-v24"`.
   A user who copy-pasted that one-liner would download and install the
   gitmap-v24 binary into the wrong repo's release flow.

2. **Dumped gitmap-v24's `CHANGELOG.md` notes into the release body** because
...
- `gitmap-v24/release/workflowfinalize.go::pushAndFinalize` —
...
- `gitmap-v24/release/workflowgithub.go::uploadToGitHub` — the release body
...
For the canonical gitmap-v24 source repos (`gitmap-v24`, `gitmap-v24`, …):
...
- `gitmap-v24/release/releaseinstallhint.go::ShouldPrintInstallHint` — the
  single source of truth for "is this the gitmap-v24 source repo?".
