# `gitmap-v25 release` must not leak gitmap-specific content into other repos' releases

## Symptom

Running `gitmap-v25 release` inside a non-gitmap repository (e.g. `img-pdf-v2`,
...
1. **Attached `release-version-vX.Y.Z.ps1` / `release-version-vX.Y.Z.sh`
   ...
   `REPO="alimtvnetwork/gitmap-v25"` and `BINARY_NAME="gitmap-v25"`.
   A user who copy-pasted that one-liner would download and install the
   gitmap-v25 binary into the wrong repo's release flow.

2. **Dumped gitmap-v25's `CHANGELOG.md` notes into the release body** because
...
- `gitmap-v25/release/workflowfinalize.go::pushAndFinalize` —
...
- `gitmap-v25/release/workflowgithub.go::uploadToGitHub` — the release body
...
For the canonical gitmap-v25 source repos (`gitmap-v25`, `gitmap-v25`, …):
...
- `gitmap-v25/release/releaseinstallhint.go::ShouldPrintInstallHint` — the
  single source of truth for "is this the gitmap-v25 source repo?".
