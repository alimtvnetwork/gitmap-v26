...
## Issue 1: `gitmap-v22 hd` Fails — docs-site Not Included in Release

### Symptom

```
PS D:\scripts-fixture> gitmap-v22 hd
...
## Issue 2: Directory Casing — `GitMap` vs `gitmap-v22`

### Symptom

The error path shows `D:\dev\GitMap\docs-site` — PascalCase `GitMap` directory.
`constants_doctor.go` correctly uses `GitMapSubdir = "gitmap-v22"` (lowercase), but
...
Ensure all references use lowercase `gitmap-v22` consistently. The `resolveBinaryDir()`
