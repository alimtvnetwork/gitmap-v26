# `gitmap-v22 cd` prints the path but does not change the current shell directory

## Ticket

Users run `gitmap-v22 cd <repo>` in an interactive shell and expect the terminal
itself to move into that repository, but the command only prints the absolute
path.

## Symptoms

1. User runs `gitmap-v22 cd gitmap-v22`.
...
`gitmap-v22 cd` is implemented as a normal CLI subcommand. A child process can
...
separate `gcd` shell helper during `gitmap-v22 setup`. That left a behavioral gap:

- `gitmap-v22 cd <repo>` still executed the real binary directly.
...
managed wrapper for **both** `gitmap-v22` and `gcd`:

1. Install a shell function named `gitmap-v22`.
2. Intercept `gitmap-v22 cd ...` and `gitmap-v22 go ...`.
3. Delegate to the real executable (`command gitmap-v22` on Bash/Zsh,
...
The wrapper uses a new managed marker so rerunning `gitmap-v22 setup` appends the
...
- `gitmap-v22/completion/cdfunction.go`
- `gitmap-v22/constants/constants_cd.go`