...
After installing gitmap-v24 via `curl | bash` on macOS (or `irm | iex` on Windows),
running `gitmap-v24` immediately returns "command not found" / "not recognized".
...
4. User types `gitmap-v24` → `zsh: command not found: gitmap-v24`.

### Windows

1. User runs `irm .../install.ps1 | iex` in PowerShell.
2. Installer adds install directory to User PATH in the registry.
3. User types `gitmap-v24` → `gitmap-v24: The term 'gitmap-v24' is not recognized`.
4. CMD and Git Bash sessions also cannot find `gitmap-v24`.
...
`gitmap-v24` to just work immediately after install.
...
```
  ✓ Installed! To start using gitmap-v24 right now, run:
...
```
  ✓ Installed! To start using gitmap-v24 right now:
...
- `gitmap-v24/scripts/install.sh` — Unix implementation
- `gitmap-v24/scripts/install.ps1` — Windows implementation
