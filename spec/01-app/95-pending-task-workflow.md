...
Every file-system operation in gitmap-v24 (delete, remove, scan, clone, pull,
...
automatically replayed via `gitmap-v24 do-pending`.
...
1. Re-execute `gitmap-v24 <CommandArgs>` as a subprocess.
...
5. `gitmap-v24 pending` lists all pending tasks with Id, type, path, reason.
6. `gitmap-v24 do-pending` retries all; `gitmap-v24 dp` is an alias.
7. `gitmap-v24 do-pending <id>` retries a single task.
