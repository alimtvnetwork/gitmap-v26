...
Every file-system operation in gitmap-v22 (delete, remove, scan, clone, pull,
...
automatically replayed via `gitmap-v22 do-pending`.
...
1. Re-execute `gitmap-v22 <CommandArgs>` as a subprocess.
...
5. `gitmap-v22 pending` lists all pending tasks with Id, type, path, reason.
6. `gitmap-v22 do-pending` retries all; `gitmap-v22 dp` is an alias.
7. `gitmap-v22 do-pending <id>` retries a single task.
