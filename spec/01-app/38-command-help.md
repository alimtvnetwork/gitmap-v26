...
Every gitmap-v22 command supports a `--help` flag that prints detailed
...
```
gitmap-v22/helptext/<command-name>.md
```

Example files:

```
gitmap-v22/helptext/scan.md
gitmap-v22/helptext/clone.md
gitmap-v22/helptext/cd.md
gitmap-v22/helptext/go-repos.md
gitmap-v22/helptext/release.md
...
```
...
- Run `gitmap-v22 scan` first to populate the database (link to scan.md)
...
A new package `gitmap-v22/helptext` bundles all `.md` files:
...
When a user runs `gitmap-v22 <command> --help`:
...
import "github.com/alimtvnetwork/gitmap-v22/gitmap-v22/helptext"
...
For full details, each entry links to `gitmap-v22/helptext/<command>.md`.
...
1. Create `gitmap-v22/helptext/` directory with all 41 `.md` files
2. Create `gitmap-v22/helptext/print.go` with `go:embed` and `Print` function
3. Create `gitmap-v22/cmd/helpcheck.go` with `checkHelp` function
...
- [ ] `gitmap-v22 scan --help` prints scan help with examples and exits 0
- [ ] `gitmap-v22 cd --help` prints cd help including prerequisites
- [ ] `gitmap-v22 go-repos -h` prints project detection help
...
- [ ] `gitmap-v22 help` continues to print the existing summary usage
