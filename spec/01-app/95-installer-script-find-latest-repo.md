...
**Scope:** Generic. Applies to any project whose repo name ends in a `-v<N>` suffix (e.g. `gitmap-v24`, `myapp-v7`, `cli-tool-v12`).
...
This guarantees a stale install URL (e.g. someone bookmarked `gitmap-v24` years ago) still pulls the user onto the latest major repo line without manual intervention.
...
| **Installer path**| Path inside the repo to the installer (e.g. `gitmap-v24/scripts/install.ps1` or `install.sh`). MUST be identical across versions. |
...
| Input URL                                          | stem        | N  |
|----------------------------------------------------|-------------|----|
| `https://github.com/alimtvnetwork/gitmap-v24`       | `gitmap-v24`    | 3  |
...
| `gitmap-v24/scripts/install.ps1`    | ✅        | ✅         | `-NoDiscovery`, `-ProbeCeiling`  |
| `gitmap-v24/scripts/install.sh`     | ✅        | ✅         | `--no-discovery`, `--probe-ceiling` |
...
## 13. Concrete Example (gitmap-v24)
...
```
  [discovery] baseline: alimtvnetwork/gitmap-v24
  [discovery] probe ceiling: 30
  [discovery] HEAD https://github.com/alimtvnetwork/gitmap-v24 ... HIT
  [discovery] HEAD https://github.com/alimtvnetwork/gitmap-v24 ... MISS (fail-fast)
  [discovery] effective: alimtvnetwork/gitmap-v24
  [discovery] delegating to https://raw.githubusercontent.com/alimtvnetwork/gitmap-v24/main/install-quick.sh
```

The user transparently lands on `gitmap-v24` even though they invoked `gitmap-v24`.
