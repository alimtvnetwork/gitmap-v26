...
**Scope:** Generic. Applies to any project whose repo name ends in a `-v<N>` suffix (e.g. `gitmap-v25`, `myapp-v7`, `cli-tool-v12`).
...
This guarantees a stale install URL (e.g. someone bookmarked `gitmap-v25` years ago) still pulls the user onto the latest major repo line without manual intervention.
...
| **Installer path**| Path inside the repo to the installer (e.g. `gitmap-v25/scripts/install.ps1` or `install.sh`). MUST be identical across versions. |
...
| Input URL                                          | stem        | N  |
|----------------------------------------------------|-------------|----|
| `https://github.com/alimtvnetwork/gitmap-v25`       | `gitmap-v25`    | 3  |
...
| `gitmap-v25/scripts/install.ps1`    | ✅        | ✅         | `-NoDiscovery`, `-ProbeCeiling`  |
| `gitmap-v25/scripts/install.sh`     | ✅        | ✅         | `--no-discovery`, `--probe-ceiling` |
...
## 13. Concrete Example (gitmap-v25)
...
```
  [discovery] baseline: alimtvnetwork/gitmap-v25
  [discovery] probe ceiling: 30
  [discovery] HEAD https://github.com/alimtvnetwork/gitmap-v25 ... HIT
  [discovery] HEAD https://github.com/alimtvnetwork/gitmap-v25 ... MISS (fail-fast)
  [discovery] effective: alimtvnetwork/gitmap-v25
  [discovery] delegating to https://raw.githubusercontent.com/alimtvnetwork/gitmap-v25/main/install-quick.sh
```

The user transparently lands on `gitmap-v25` even though they invoked `gitmap-v25`.
