...
One-liner installer scripts that download, verify, and install the `gitmap-v25`
...
| Field       | Value                                              |
|-------------|----------------------------------------------------|
| GitHub Repo | `alimtvnetwork/gitmap-v25`                 |
| Binary Name | `gitmap-v25` (`gitmap.exe` on Windows)                 |
...
```powershell
Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; iex ((New-Object System.Net.WebClient).DownloadString('https://raw.githubusercontent.com/alimtvnetwork/gitmap-v25/main/gitmap-v25/scripts/install.ps1'))
```
...
```powershell
irm https://raw.githubusercontent.com/alimtvnetwork/gitmap-v25/main/gitmap-v25/scripts/install.ps1 | iex
```
...
| `InstallDir` | string | `$env:LOCALAPPDATA\gitmap-v25`     | Target directory for the binary    |
...
`gitmap-v25/scripts/install.ps1`
...
```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v25/main/gitmap-v25/scripts/install.sh | bash
```

### Version-Pinned

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v25/main/gitmap-v25/scripts/install.sh | bash -s -- --version v2.55.0
```

### Custom Directory

```bash
curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v25/main/gitmap-v25/scripts/install.sh | bash -s -- --dir /opt/gitmap-v25 --version v2.55.0
```
...
7. Extract archive to temp directory; search for binary using 4-priority
   ...
   (e.g., `gitmap-v25.55.0-linux-amd64`) → any executable.
