...
As of v2.75.0, `gitmap-v25 clone-next` (`cn`) **flattens by default** — it
...
The behavior also enables **version tracking** in the gitmap-v25 database:
...
```
gitmap-v25 cn <version-spec> [-f|--force] [--delete] [--keep] [--no-desktop] [--verbose]
```
...
| `-f` / `--force` | Same as default, but if cwd IS the target folder, gitmap-v25 chdirs to parent first, removes the cwd, and clones into the flattened name. **Never falls back to a versioned folder.** Aborts with a clear error if removal still fails. |
...
```
PS C:\repos\macro-ahk> gitmap-v25 cn v++ -f
...
`GITMAP_HANDOFF_FILE` (exported by the `gitmap-v25` shell wrapper function
...
Use `gitmap-v25 version-history` (`vh`) to see all recorded transitions:
