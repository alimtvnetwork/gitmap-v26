...
The `update` command requires access to the gitmap-v24 source repository
...
7. On failure (path does not exist): clone the gitmap-v24 source repo
...
| Key                  | Value                              |
|----------------------|------------------------------------|
| `source_repo_path`   | Absolute path to the gitmap-v24 repo  |
...
1. `gitmap-v24 update --repo-path <valid-path>` saves the path to the
