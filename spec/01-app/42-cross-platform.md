...
gitmap-v25 supports Windows, Linux, and macOS with feature parity across
...
The `gitmap-v25/Makefile` provides a standard interface wrapping `run.sh`:
...
3. Run `gitmap-v25 release` with `--compress --checksums`
...
- Binary names: `gitmap-v25` on Unix, `gitmap.exe` on Windows
