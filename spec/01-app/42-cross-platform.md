...
gitmap-v22 supports Windows, Linux, and macOS with feature parity across
...
The `gitmap-v22/Makefile` provides a standard interface wrapping `run.sh`:
...
3. Run `gitmap-v22 release` with `--compress --checksums`
...
- Binary names: `gitmap-v22` on Unix, `gitmap.exe` on Windows
