...
> Spec for `gitmap-v25 self-install` and `gitmap-v25 self-uninstall` — manage
> the gitmap-v25 binary itself (NOT to be confused with `gitmap-v25 install` /
> `gitmap-v25 uninstall`, which manage third-party tools).

---

## Why two new commands?

`gitmap-v25 install` and `gitmap-v25 uninstall` were already taken by the
third-party tool installer (npp, vscode, dev tools). Users asked for a
way to wipe the gitmap-v25 binary itself and re-install it from the same
session. To avoid breaking the existing tool installer, we added two
new top-level verbs:

| Command              | Scope                                                     |
|----------------------|-----------------------------------------------------------|
| `gitmap-v25 install`     | Install a third-party tool (existing, unchanged)          |
| `gitmap-v25 uninstall`   | Uninstall a third-party tool (existing, unchanged)        |
| `gitmap-v25 self-install`   | Install / re-install the gitmap-v25 binary                |
| `gitmap-v25 self-uninstall` | Remove the gitmap-v25 binary, data dir, PATH snippet, completion |
...
1. **Binary + deploy artefacts** — anything under the directory that
   ...
   `gitmap-v25`, `gitmap.exe`, `gitmap-handoff-*`, `*.old` backups,
   `gitmap-completion.*`.
2. **`.gitmap/` data dir** — SQLite DB, profiles, scan history. Skip
   with `--keep-data`.
3. **PATH snippet** — strips the `# gitmap-v25 shell wrapper v* - managed
   by *. Do not edit manually.` … `# gitmap-v25 shell wrapper v* end` block
...
- **Windows**: `D:\gitmap-v25`
- **Unix**: `$HOME/.local/bin/gitmap-v25`
...
| File                                       | Role                                                 |
|--------------------------------------------|------------------------------------------------------|
| `gitmap-v25/constants/constants_selfinstall.go` | Command IDs, messages, defaults, flag descriptions  |
| `gitmap-v25/scripts/embed.go`                  | `go:embed` of install.ps1, install.sh, uninstall.ps1 |
| `gitmap-v25/cmd/selfinstall.go`                | Entry, flag parsing, prompt, script loader, exec     |
| `gitmap-v25/cmd/selfuninstall.go`              | Entry, flag parsing, confirm, executeSelfUninstall   |
| `gitmap-v25/cmd/selfuninstallparts.go`         | Removers: deploy dir, completion, profile snippet    |
| `gitmap-v25/cmd/selfuninstallhandoff.go`       | Windows temp-copy handoff + self-delete scheduler    |
| `gitmap-v25/helptext/self-install.md`          | User-facing help                                     |
| `gitmap-v25/helptext/self-uninstall.md`        | User-facing help                                     |
...
- `gitmap-v25/scripts/install.ps1`, `install.sh`, `uninstall.ps1` — the
