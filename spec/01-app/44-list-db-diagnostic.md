...
The `gitmap-v22 ls` command should provide transparent feedback about
...
```
  ...
  No repos tracked. Run 'gitmap-v22 scan' first.
```
...
- **Given** `gitmap-v22 ls --verbose` with repos, **then** DB path is printed
  above the repo table.
- **Given** `gitmap-v22 ls` with zero repos, **then** DB path is printed
