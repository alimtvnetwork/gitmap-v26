...
The `gitmap-v25 ls` command should provide transparent feedback about
...
```
  ...
  No repos tracked. Run 'gitmap-v25 scan' first.
```
...
- **Given** `gitmap-v25 ls --verbose` with repos, **then** DB path is printed
  above the repo table.
- **Given** `gitmap-v25 ls` with zero repos, **then** DB path is printed
