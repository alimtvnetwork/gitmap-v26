# 31 — `gitmap-v25 open` (alias `op`)

> **Status:** specified — implemented in `gitmap-v25/cmd/open.go`.
> **Schema dependency:** v25 (idempotency stamps on `Repo`).

## Purpose

`gitmap-v25 open` is the "I just `cd`'d into this repo, get me into both
...
muscle-memory pair of `gitmap-v25 inject .` followed by `code .`.
...
```
gitmap-v25 open              # alias: op
gitmap-v25 open --force      # bypass idempotency stamps; alias: -f
```
...
|---|---|---|---|---|---|
| `gitmap-v25 inject [path]` | yes (if origin) | yes | yes | yes | per-tool stamp |
| `gitmap-v25 open` | yes (if origin) | yes | yes | no | per-tool stamp |
| `gitmap-v25 code` | no | no | yes | no | none |
| `gitmap-v25 github-desktop` | no | yes | no | no | none |
...
- `gitmap-v25/cmd/open.go` — implementation.
- `gitmap-v25/cmd/inject.go` — sibling command, shares
