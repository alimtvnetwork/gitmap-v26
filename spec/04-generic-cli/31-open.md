# 31 — `gitmap-v22 open` (alias `op`)

> **Status:** specified — implemented in `gitmap-v22/cmd/open.go`.
> **Schema dependency:** v25 (idempotency stamps on `Repo`).

## Purpose

`gitmap-v22 open` is the "I just `cd`'d into this repo, get me into both
...
muscle-memory pair of `gitmap-v22 inject .` followed by `code .`.
...
```
gitmap-v22 open              # alias: op
gitmap-v22 open --force      # bypass idempotency stamps; alias: -f
```
...
|---|---|---|---|---|---|
| `gitmap-v22 inject [path]` | yes (if origin) | yes | yes | yes | per-tool stamp |
| `gitmap-v22 open` | yes (if origin) | yes | yes | no | per-tool stamp |
| `gitmap-v22 code` | no | no | yes | no | none |
| `gitmap-v22 github-desktop` | no | yes | no | no | none |
...
- `gitmap-v22/cmd/open.go` — implementation.
- `gitmap-v22/cmd/inject.go` — sibling command, shares
