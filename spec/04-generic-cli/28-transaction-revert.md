...
Every gitmap-v24 operation that **mutates the on-disk filesystem** is recorded
...
gitmap-v24 state, so it is automatically anchored to the binary path
...
## 5. CLI surface — extend `gitmap-v24 revert`

The legacy `gitmap-v24 revert <version>` flow (rolls back the gitmap-v24
...
Help text lives at `gitmap-v24/helptext/revert.md` (extended, not replaced).
...
- `gitmap-v24 revert --prune-txn` lets users force a prune cycle.

## 8. Concurrency

- The existing `gitmap.lock` advisory file lock guards every Begin
  / Commit / Revert call so two gitmap-v24 processes never interleave
  transaction rows.
...
- A full UI under `gitmap-v24 txn ...` — left as a follow-up; the `revert`
  flag surface is sufficient for v1.

## 10. Implementation references

- Schema: `gitmap-v24/constants/constants_transaction.go`
- Tables: `gitmap-v24/store/transaction.go`, `gitmap-v24/store/transaction_file.go`
- Engine: `gitmap-v24/txn/begin.go`, `gitmap-v24/txn/snapshot.go`, `gitmap-v24/txn/revert.go`, `gitmap-v24/txn/prune.go`
- CLI: `gitmap-v24/cmd/revert.go` (extended), `gitmap-v24/cmd/revert_txn.go` (new)
- Help: `gitmap-v24/helptext/revert.md` (extended)
