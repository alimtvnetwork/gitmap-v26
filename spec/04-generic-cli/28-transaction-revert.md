...
Every gitmap-v22 operation that **mutates the on-disk filesystem** is recorded
...
gitmap-v22 state, so it is automatically anchored to the binary path
...
## 5. CLI surface — extend `gitmap-v22 revert`

The legacy `gitmap-v22 revert <version>` flow (rolls back the gitmap-v22
...
Help text lives at `gitmap-v22/helptext/revert.md` (extended, not replaced).
...
- `gitmap-v22 revert --prune-txn` lets users force a prune cycle.

## 8. Concurrency

- The existing `gitmap.lock` advisory file lock guards every Begin
  / Commit / Revert call so two gitmap-v22 processes never interleave
  transaction rows.
...
- A full UI under `gitmap-v22 txn ...` — left as a follow-up; the `revert`
  flag surface is sufficient for v1.

## 10. Implementation references

- Schema: `gitmap-v22/constants/constants_transaction.go`
- Tables: `gitmap-v22/store/transaction.go`, `gitmap-v22/store/transaction_file.go`
- Engine: `gitmap-v22/txn/begin.go`, `gitmap-v22/txn/snapshot.go`, `gitmap-v22/txn/revert.go`, `gitmap-v22/txn/prune.go`
- CLI: `gitmap-v22/cmd/revert.go` (extended), `gitmap-v22/cmd/revert_txn.go` (new)
- Help: `gitmap-v22/helptext/revert.md` (extended)
