...
state-mutating gitmap-v25 operation must produce so that revert is
...
- Adding a new `ActionKind` requires:
  ...
  3. Forward + reverse handlers in `gitmap-v25/txn/actions/`.
...
- Engine: `gitmap-v25/txn/actions/{plan,apply,reverse}.go`, one dispatch
  table keyed by `ActionKind`.
- CLI surface unchanged — `gitmap-v25 revert` still drives everything via
