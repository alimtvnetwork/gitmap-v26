...
state-mutating gitmap-v24 operation must produce so that revert is
...
- Adding a new `ActionKind` requires:
  ...
  3. Forward + reverse handlers in `gitmap-v24/txn/actions/`.
...
- Engine: `gitmap-v24/txn/actions/{plan,apply,reverse}.go`, one dispatch
  table keyed by `ActionKind`.
- CLI surface unchanged — `gitmap-v24 revert` still drives everything via
