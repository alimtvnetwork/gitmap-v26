...
state-mutating gitmap-v22 operation must produce so that revert is
...
- Adding a new `ActionKind` requires:
  ...
  3. Forward + reverse handlers in `gitmap-v22/txn/actions/`.
...
- Engine: `gitmap-v22/txn/actions/{plan,apply,reverse}.go`, one dispatch
  table keyed by `ActionKind`.
- CLI surface unchanged — `gitmap-v22 revert` still drives everything via
