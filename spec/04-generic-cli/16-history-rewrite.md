...
> `gitmap-v25/cmd/historyrewrite*.go`. Research backing this spec:
...
```
gitmap-v25 history-purge <path> [<path> ...] [flags]
gitmap-v25 hp            <path> [<path> ...] [flags]

gitmap-v25 history-pin   <path> [<path> ...] [flags]
gitmap-v25 hpin          <path> [<path> ...] [flags]
```
...
- **purge-secret**: build a temp git repo with 5 commits incl.
  `secret.env`, run `gitmap-v25 hp secret.env --yes --no-push`, assert
  `git log --all -- secret.env` is empty and `git count-objects -v`
  shrunk.
- **pin-current**: build a temp git repo where `X` has 3 historical
  states, run `gitmap-v25 hpin X --yes --no-push`, assert the verification
