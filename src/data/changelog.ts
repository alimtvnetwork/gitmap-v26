...
export const changelog: ChangelogEntry[] = [
  ...
  },
  {
    version: "v5.40.0",
    date: "2026-05-19",
    subtitle: "fix-repo auto-backup + new `gitmap-v24 undo` (alias `ud`) restore command",
    items: [
      "**Auto-backup.** Every `gitmap-v24 fix-repo` write (non-dry-run) now snapshots the pre-rewrite copy of each modified file to `.gitmap/backup/<repo>/v<current>/fix-repo/<UTC-ts>/files/<rel>` alongside a `manifest.json` index. Untouched files are never copied; dry-run never creates a snapshot.",
      "**New command:** `gitmap-v24 undo` (alias `ud`) restores the latest snapshot for the current repo + current version. Subcommands: `--list` (snapshots newest-first, latest marked `*`), `--snapshot <ts>` (pick a specific stamp), `--dry-run` (preview).",
      "**Scoping:** snapshots live under `<repo>/v<current>` so an undo inside `gitmap-v24` can never touch a `gitmap-v24` snapshot. Manifest carries `schemaVersion`, `repo`, `currentVersion`, `timestamp`, `gitmapVersion`, `files[]`.",
      "**Code:** new `gitmap-v24/cmd/fixrepo_backup.go` (`fixRepoBackupSession`, lazy mkdir, idempotent per rel — first observation wins), new `gitmap-v24/cmd/undo.go`, new `gitmap-v24/constants/constants_undo.go`. `rewriteOneFile` split into pure-compute + `persistRewrittenFile` so backup runs strictly BEFORE the disk write. `CmdUndo` / `CmdUndoAlias` wired in `roottooling.go` and registered in `cmd_constants_test.go`. Help text: `gitmap-v24/helptext/undo.md`.",
      ...
      "Pinned: README pinned-version block + version matrix moved to **v5.40.0**. Synced `gitmap-v24/constants/constants.go` (`Version = \"5.40.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.40.0\"`).",
    ],
  },
  {
    version: "v5.39.0",
    ...
    items: [
      "**New flag.** `gitmap-v24 fix-repo --restrict no-version` (short: `gitmap-v24 fr -2 -r nv`) suppresses the v1→v2 bare-base rewrite so ONLY `{base}-vN` tokens are touched. Bare `{base}` occurrences are left alone even during a v1→v2 bump.",
      ...
      "`gitmap-v24 help` now lists `--restrict <mode>` under 'Fix-repo flags:' with two copy-pasteable examples (full + short form).",
      ...
      "Pinned: README pinned-version block + version matrix moved to **v5.39.0**. Synced `gitmap-v24/constants/constants.go` (`Version = \"5.39.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.39.0\"`).",
    ],
  },
  {
    version: "v5.38.0",
    date: "2026-05-19",
    subtitle: "fix-repo bare-base rewrite restricted to v1→v2 (no more corrupting bare `gitmap-v24` at v3+)",
    items: [
      "**Critical fix.** Running `gitmap-v24 fix-repo` inside a `-v3` (or higher) repo no longer rewrites bare `{base}` tokens. Before v5.38.0, `fix-repo` inside `gitmap-v24` would rewrite every standalone mention of `gitmap-v24` (binary names, package identifiers, brand strings, unrelated `https://github.com/owner/gitmap-v24` URLs) to `gitmap-v24` — silently corrupting the working tree.",
      ...
      "Regression tests: `TestApplyAllTargets_BareBase_SkippedAtV3Plus` and `TestApplyAllTargets_BareBase_SkippedAtV4WithV1InTargets` in `gitmap-v24/cmd/fixrepo_rewrite_barebase_test.go` lock in the new behavior. The existing v1→v2 test (`TestApplyAllTargets_BareBase_V1To2`) is unchanged.",
      "Single-line change in `gitmap-v24/cmd/fixrepo_rewrite.go` (`applyAllTargets`): `if n == 1` → `if n == 1 && current == 2`.",
      "Pinned: README pinned-version block + version matrix moved to **v5.38.0**. Synced `gitmap-v24/constants/constants.go` (`Version = \"5.38.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.38.0\"`).",
    ],
  },
  {
    version: "v5.37.0",
    ...
    items: [
      "Bare `gitmap-v24` and `gitmap-v24 help` now open with a magenta/cyan banner (`🗺  gitmap-v24 vX.Y.Z — Git repo discovery, cloning & release toolkit`) and close with a build-info footer showing the installed version, source repo origin URL, current branch, and the last commit (`shortSHA · subject · relative-date`).",
      ...
      "New file `gitmap-v24/cmd/rootusagefooter.go` (helper `printUsageFooter`) wired into `printUsage`. Header constant `UsageHeaderFmt` rewritten in `gitmap-v24/constants/constants_cli.go`.",
      "Reminder for the `gitm` alias: it ships in the shell wrapper installed by `gitmap-v24 setup`. The installer auto-runs setup (v5.18+), but stale installs require a one-time `gitmap-v24 setup` + shell reload to pick up the new function.",
      "Pinned: README pinned-version block + version matrix moved to **v5.37.0**. Synced `gitmap-v24/constants/constants.go` (`Version = \"5.37.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.37.0\"`).",
    ],
  },
  {
    version: "v5.36.0",
    date: "2026-05-19",
    subtitle: "PowerShell `gitmap-v24 cd` wrapper: bulletproof `[string]` cast against `Set-Location` Object[] binding",
    items: [
      "Fix: stale-profile users hit `Set-Location : Cannot convert 'System.Object[]' to the type 'System.String' required by parameter 'LiteralPath'` when running `gitmap-v24 cd <slug>` — even after the v5.17.0 `Out-String | Trim` fix — because PowerShell could still bind `$dest` as `Object[]` in some pipelines.",
      "Hardened all four PowerShell wrapper sites: `gitmap-v24/constants/constants_cd.go` (gcd + gitmap-v24 fns), `constants_pathsnippet.go` (setup-written profile snippet), `constants_cd_shim.go` (template), and `gitmap-v24/scripts/install.ps1` (both the one-liner profile wrapper and the multi-line shim template).",
      ...
      "Action required for existing users: re-run `gitmap-v24 setup` (or reinstall) and reload the shell — the marker block `# gitmap-v24 shell wrapper v2` / `# gitmap-v24 command wrapper v1` is rewritten in-place by setup.",
      "Pinned: README pinned-version block + version matrix moved to **v5.36.0**. Synced `gitmap-v24/constants/constants.go` (`Version = \"5.36.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.36.0\"`).",
    ],
  },
  {
    version: "v5.35.0",
    ...
    items: [
      ...
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.35.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.35.0\"`).",
    ],
  },
  {
    version: "v5.31.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v24 pull-release-cd` / `prc` — multi-repo one-shot release",
    items: [
      "Added: `gitmap-v24 pull-release-cd` (alias `prc`) — multi-repo, one-shot pull-release runner. Accepts a comma-separated list of `<name-or-url> <version>` pairs (e.g. `gitm prc gitmap-v24 v5.31.0, marco v2.5.0, https://github.com/me/url-git v3.1.0`) and, for each entry, chdirs into the target repo and spawns `gitmap-v24 pull-release <version> -y` as an isolated subprocess.",
      "Added: URL tokens (containing `://` or starting with `git@`) are cloned first via a `gitmap-v24 clone <url>` subprocess; the derived slug (URL's last path segment minus `.git`) is then resolved against the gitmap-v24 DB for the subsequent pull-release.",
      ...
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.31.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.31.0\"`).",
    ],
  },
  {
    version: "v5.30.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v24 push` auto `pull --rebase` + retry on non-fast-forward rejection",
    items: [
      "Added: `gitmap-v24 push` now detects git's non-fast-forward rejection (`[rejected] ... (fetch first)` / `failed to push some refs`), automatically runs `git pull --rebase`, and retries the push once — no more manual `git pull` → `git push` cycle after `gitmap-v24 push -ssh`.",
      "Behavior: stderr is tee'd so users still see git's original rejection message live; the auto-retry path prints `↻ push rejected (non-fast-forward) — auto-running 'git pull --rebase' and retrying`. If rebase hits conflicts, the original git exit code is propagated and a hint to resolve + re-run `gitmap-v24 push` is printed.",
      "Test fix: registered `CmdPush` / `CmdPushAlias` in `topLevelCmds()` parity registry and added `gitmap-v24/helptext/push.md` to satisfy `TestEveryCmdIDHasHelpFile` (both surfaced after the v5.29.0 push command landed).",
      "Pinned: README pinned-version block + version matrix moved to **v5.30.0**.",
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.30.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.30.0\"`).",
    ],
  },
  {
    version: "v5.29.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v24 push` + `--ssh` / `--https` transport flags on push & pull",
    items: [
      "Added: `gitmap-v24 push` (alias `ph`) — runs `git push` in the current repo with full stdin/stdout/stderr forwarding and exit-code propagation, mirroring the v5.28.0 `gitmap-v24 pull` cwd short-circuit.",
      "Added: `--ssh` / `-ssh` / `--sh` and `--https` / `-https` / `--ht` transport flags on both `gitmap-v24 push` and `gitmap-v24 pull` — rewrites `remote.origin.url` to the requested transport and **persists** the change via `git remote set-url origin`, so subsequent plain `git push` / `git pull` invocations keep the new transport.",
      "Added: when both flags are set, `--ssh` wins with a one-line stderr warning (mirrors `gitmap-v24 clone` semantics from spec 110). Unrecognised origin URLs fail-open — a warning is printed but git push/pull still runs.",
      "Added: extra positional args after the flags forward verbatim, so `gitmap-v24 push --ssh origin main` runs `git push origin main` against the freshly-rewritten SSH origin.",
      "Added: end-to-end tests in `gitmap-v24/cmd/pushpull_transport_e2e_test.go` cover HTTPS↔SSH conversion + persistence, idempotent no-op, `--ssh` winning conflict, and unrecognised-URL fail-open — using a real `git` binary against a temp bare repo (skipped when git is missing).",
      "Spec: `spec/01-app/111-push-pull-transport-flags.md`.",
      "Pinned: README pinned-version block + version matrix moved to **v5.29.0**.",
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.29.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.29.0\"`).",
    ],
  },
  {
    version: "v5.28.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v24 pull` works like `git pull` in a repo + new `gitm` short alias",
    items: [
      "Added: `gitmap-v24 pull` with no args now short-circuits to a plain `git pull` when run inside a git repo — stdin/stdout/stderr forwarded, underlying exit code propagated. Slug / `--group` / `--all` / aliased-repo modes are unchanged.",
      "Added: `gitm` shell alias — Bash / Zsh / PowerShell wrappers installed by `gitmap-v24 setup` now also define `gitm` as a thin forwarder to `gitmap-v24`, so `gitm pull`, `gitm cd <name>`, `gitm clone <url>` all behave identically. Re-run `gitmap-v24 setup` (or reinstall) to pick up the new wrapper block.",
      "Confirmed: `gitmap-v24 setup` auto-run after install has shipped since v5.18.0 — both `install.ps1` and `install.sh` call `gitmap-v24 setup` as a non-fatal final step, so `gitm` is registered automatically on fresh installs.",
      "Pinned: README pinned-version block + version matrix moved to **v5.28.0**.",
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.28.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.28.0\"`).",
    ],
  },
  {
    version: "v5.27.0",
    date: "2026-05-19",
    subtitle: "`gitmap-v24 cfrp` / `cfr` now honour `--ssh` / `--https`",
    items: [
      "Fixed: `gitmap-v24 cfrp <url> --ssh` (and `cfr`) was ignoring the transport flag — `parseCloneFixRepoArgs` only recognised `--no-vscode-sync` and `--require-version`, so the HTTPS URL was passed straight to the in-process clone.",
      "Fixed: URL is now rewritten via `ConvertURLToSSH` / `ConvertURLToHTTPS` before the clone runs, mirroring `gitmap-v24 clone --ssh` exactly.",
      "Added: accepts `--ssh`, `-ssh`, `--sh`, `-sh`, `--https`, `-https`, `--ht`, `-ht` (single- and double-dash, plus the same short aliases as `gitmap-v24 clone`). When both `--ssh` and `--https` are set, `--ssh` wins with a stderr warning.",
      ...
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.27.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.27.0\"`).",
    ],
  },
  {
    version: "v5.26.0",
    ...
    items: [
      ...
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.26.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.26.0\"`).",
      "Rolled up: `gitmap-v24 clone <url> --ssh` (and every other bool clone flag) is honoured regardless of position — `parseCloneFlags` routes through `reorderFlagsBeforeArgs` so a trailing `--ssh` no longer slips past Go's `flag` package.",
  ...
  },
  {
    version: "v5.24.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v24 clone --ssh` now works when the flag follows the URL",
    items: [
      "Fixed: `gitmap-v24 clone <url> --ssh` (and `--https`, `--no-replace`, every other bool flag) was silently ignored when written AFTER the positional URL. Go's `flag` package stops parsing at the first non-flag, so the trailing `--ssh` never reached `applyURLSchemeFlags` and the HTTPS URL was cloned as-is.",
      "Fixed: `parseCloneFlags` now routes through `reorderFlagsBeforeArgs` (the same helper used by `release`, `clone-next`, `clone-from`, `commit-transfer`, etc.), so flags are honoured regardless of position. `gitmap-v24 clone --ssh <url>`, `gitmap-v24 clone <url> --ssh`, and `gitmap-v24 clone <url> --ssh --no-replace` all behave identically.",
      ...
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.24.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.24.0\"`).",
    ],
  },
  {
    version: "v5.23.0",
    ...
    items: [
      "Added: `install.ps1` and `install.sh` now live at the **repository root**. New one-liners: Windows `irm https://raw.githubusercontent.com/alimtvnetwork/gitmap-v24/main/install.ps1 | iex` and macOS/Linux `curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v24/main/install.sh | sh`. Shorter, more discoverable — no more `gitmap-v24/scripts/` segment for end users.",
      "Kept: `gitmap-v24/scripts/install.{ps1,sh}` are unchanged source-of-truth (consumed by `go:embed` for `gitmap-v24 self-install`, the `release-version` generators, CI workflows, smoke tests, and ~50 spec/memory docs). Root files are byte-identical copies — same checksum contract, same behavior.",
      "Pinned: README pinned-version block + version matrix moved to **v5.23.0**. All 11 install URLs in the README rewritten from `main/gitmap-v24/scripts/install.{ps1,sh}` → `main/install.{ps1,sh}`.",
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.23.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.23.0\"`).",
    ],
  },
  {
    version: "v5.22.0",
    date: "2026-05-18",
    subtitle: "Pin bump — rolls up v5.21.0 `gitmap-v24 ssh view` / `copy` / `create`",
    items: [
      ...
      "Synced: `gitmap-v24/constants/constants.go` (`Version = \"5.22.0\"`) and `src/constants/index.ts` (`VERSION = \"v5.22.0\"`).",
      "No behavior changes since v5.21.0 — this release re-pins the new `gitmap-v24 ssh view` / `copy` / `create` surface (clipboard-aware, soft-fails when no `clip` / `pbcopy` / `wl-copy` / `xclip` / `xsel` is on PATH) as the stable build.",
    ],
  },
  {
    version: "v5.21.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v24 ssh` gets `view` / `copy` / `create` subcommands (clipboard-aware)",
    items: [
      "Added: `gitmap-v24 ssh view <key>` (aliases `v`, existing `cat`) prints the public key to stdout. Same output as `ssh cat` — just a more discoverable verb alongside `create` / `copy` / `list` / `delete`.",
      "Added: `gitmap-v24 ssh copy <key>` (alias `cp`) prints the public key AND pushes it to the OS clipboard in one shot. Picks the right tool per OS: `clip` on Windows, `pbcopy` on macOS, `wl-copy` → `xclip -selection clipboard` → `xsel --clipboard --input` on Linux (first one found). If no clipboard tool is available, the key is still printed and a one-line warning is emitted to stderr — the command never fails.",
      "Added: `gitmap-v24 ssh create <flags>` is an explicit alias for the default `gitmap-v24 ssh` (generate). All existing `--name` / `--path` / `--email` / `--force` flags work unchanged.",
      "Files: `gitmap-v24/constants/constants_ssh.go` (new subcommand consts + clipboard messages); `gitmap-v24/cmd/ssh.go` (dispatch); `gitmap-v24/cmd/sshcopy.go` (new — `runSSHCopy`, `writeClipboard`, `resolveClipboardTool`); `gitmap-v24/helptext/ssh.md`.",
    ],
  },
  {
    version: "v5.20.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v24 clone --ssh` / `--https` coerce every URL into the requested transport before git runs",
    items: [
      "Added: `gitmap-v24 clone --ssh` rewrites every recognised Git URL into its `git@host:owner/repo.git` SSH-shorthand form before `git clone` is invoked. HTTPS (`https://github.com/owner/repo`) and `ssh://git@host[:port]/owner/repo` URLs are both converted; already-shorthand URLs are normalized (`.git` suffix appended). The flag flows through the multi-URL form too — `clone url1,url2,url3 --ssh` converts the whole batch in one shot.",
      "Added: `gitmap-v24 clone --https` is the symmetric counterpart — forces every URL into `https://host/owner/repo.git` form. Useful in CI/headless environments where the SSH agent isn't unlocked. `--ssh` and `--https` are mutually exclusive; when both are set, `--ssh` wins and a one-line stderr warning is printed.",
      ...
      "Files: `gitmap-v24/cmd/cloneurlconvert.go` (new — `ConvertURLToSSH`, `ConvertURLToHTTPS`, `httpsToSSHShorthand`, `shorthandToHTTPS`, `sshSchemeToShorthand`, `sshSchemeToHTTPS`, `ensureGitSuffix`); `gitmap-v24/cmd/rootflags.go` (UseSSH/UseHTTPS on CloneFlags, `--ssh`/`--https` registration); `gitmap-v24/cmd/clone.go` (`applyURLSchemeFlags`, wired into `runClone`).",
  ...
  },
  {
    version: "v5.19.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v24 rp` (release-pending) rejects version args + canonical command banner clears `rp` vs `pr` alias confusion",
    items: [
      "Fixed: `gitmap-v24 rp v3.1` no longer silently runs `release-pending` and releases unrelated versions (e.g. an orphaned `v2.233.0` metadata file). The old code parsed `v3.1` into `flag.Args()` and ignored it, so users who meant `pr v3.1` (pull-release) ended up releasing whatever was pending. `runReleasePending` now scans positional args for a version-shaped token (regex `^v?\\d+(\\.\\d+){0,2}(-...)?$`) and exits 2 with a precise suggestion pointing at `gitmap-v24 pr <ver>` or `gitmap-v24 release <ver>`.",
      "Added: canonical command banner at the start of `gitmap-v24 release-pending` and `gitmap-v24 pull-release` — when a short alias (`rp`, `pr`) is invoked, the resolved canonical name + alias print to stderr first (`→ Running: gitmap-v24 release-pending  (alias: rp)`) so it's immediately obvious which pipeline is running. New reusable helper `printCanonicalCmdBanner` in `gitmap-v24/cmd/releasepending.go`.",
      "Files: `gitmap-v24/cmd/releasepending.go` (runReleasePending, rejectVersionArgOnPending, printCanonicalCmdBanner, versionLikeArgPattern); `gitmap-v24/cmd/releasepull.go` (runReleasePull banner call).",
    ],
  },
  {
    version: "v5.18.0",
    date: "2026-05-18",
    subtitle: "Auto-run `gitmap-v24 setup` after install and on first `gitmap-v24 cd` when the shell wrapper isn't loaded",
    items: [
      "Install scripts (`gitmap-v24/scripts/install.sh` + `gitmap-v24/scripts/install.ps1`) now invoke `<bin_path> setup` as a final non-fatal step after `verify_installation` / `Invoke-InstallVerification`, so a fresh `irm … | iex` or `curl … | sh` lands the user with shell wrapper (`gcd` / `gitmap-v24` profile function) + completions installed — no second manual command. Setup is idempotent (marker `# gitmap-v24 shell wrapper v2`); failures print a yellow `(setup auto-run skipped …)` line and continue.",
      "`gitmap-v24 cd` self-heals when the shell wrapper isn't loaded: `warnIfNoWrapper` in `gitmap-v24/cmd/setupverify.go` now auto-runs the full `gitmap-v24 setup` (not just `InstallCDFunction`) so the very first `gitmap-v24 cd <repo>` after a fresh install installs the wrapper + completions automatically. Existing stderr reload tip (`. $PROFILE` / `source ~/.zshrc`) still prints so the user knows why the *next* `cd` will actually move the parent shell. Recover-guarded — a setup panic never breaks the cd output.",
      "Closes the first-run UX gap where users hit `! Shell wrapper not active — 'gitmap-v24 cd' printed the path but cannot change your directory.` because `gitmap-v24 setup` was a separate manual step.",
    ],
  },
  {
    version: "v5.17.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v24 cd` PowerShell wrapper: coerce stdout to a single string (fixes `Set-Location : Cannot convert 'System.Object[]'`)",
    items: [
      "`gitmap-v24 cd <repo>` crashed in PowerShell with `Set-Location : Cannot convert 'System.Object[]' to the type 'System.String' required by parameter 'LiteralPath'` whenever the captured stdout parsed as a multi-element array (extra line, CRLF artifact, or auxiliary write turned `$dest` into `System.Object[]`).",
      "All four PowerShell wrapper templates now collapse the captured output with `(& $real … | Out-String).Trim()` so `$dest` is always a single trimmed string before `Test-Path` / `Set-Location`: `gitmap-v24/constants/constants_cd.go` (CDFuncPowerShell `gcd` + `gitmap-v24`), `gitmap-v24/constants/constants_pathsnippet.go` (PathSnippetPwshFmt `Invoke-GitmapAndSetLocation`), `gitmap-v24/constants/constants_cd_shim.go` (gitmap.ps1 shim), `gitmap-v24/scripts/install.ps1` (Get-GitmapCommandWrapperBlock + Get-GitmapPowerShellShimContent).",
      "Users hit by the v5.16.0-and-earlier wrapper must re-run `gitmap-v24 setup` (or re-install) so their `Microsoft.PowerShell_profile.ps1` snippet is rewritten with the fixed body.",
    ],
  },
  {
    version: "v5.16.0",
    date: "2026-05-18",
    subtitle: "`gitmap-v24 release` no longer leaks gitmap-specific content (CHANGELOG body + release-version.{ps1,sh} snapshots) into other repos' releases",
    items: [
      "Release body: `uploadToGitHub` in `gitmap-v24/release/workflowgithub.go` now starts with an empty body and only calls `DetectChangelog()` + `AppendPinnedInstallSnippet` when `ShouldPrintInstallHint(getRemoteURL())` is true. Non-gitmap repos get a tag-only release with an empty body — no more gitmap-v24 CHANGELOG.md notes dumped into unrelated release pages.",
      "`release-version-vX.Y.Z.{ps1,sh}` snapshot assets are no longer attached to non-gitmap releases. Those snapshots hard-code `REPO=\"alimtvnetwork/gitmap-v24\"` and `BINARY_NAME=\"gitmap-v24\"`. `pushAndFinalize` in `gitmap-v24/release/workflowfinalize.go` wraps `buildReleaseVersionSnapshots` in the same `ShouldPrintInstallHint` gate.",
  ...
  },
  {
    ...
    items: [
      "Markdown help renderer (`gitmap-v24/render`) now color-codes `# comments` inside code fences in green and credential-like tokens (`API_KEY`, `GITMAP_TOKEN`, etc.) and the `hd` alias in magenta.",
  ...
  },
  {
    ...
    items: [
      ...
      "Synced version constants so the regression test in `src/test/version-sync.test.ts` stays green: `gitmap-v24/constants/constants.go` `Version` `5.10.0` → `5.13.0`, and `src/constants/index.ts` web `VERSION` `v5.10.0` → `v5.13.0`.",
      ...
      "Files: `README.md` (pinned-version block + 4 CI badge URLs), `gitmap-v24/constants/constants.go` (`Version`), `src/constants/index.ts` (`VERSION`), `CHANGELOG.md` (v5.13.0 entry), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.14.0",
    ...
    items: [
      "Added a new `--strict` (PowerShell alias `-Strict`) flag to `gitmap-v24 fix-repo` / `fr` and to `fix-repo.ps1`. When supplied, the command runs `go test` against the unique set of Go packages whose `.go` files were modified, AFTER the rewrite step and AFTER the v4.8.0 `gofmt -w` post-step, and exits with the new code **9** (`FixRepoExitTestsFailed`) on any test failure. Off by default — `gitmap-v24 fix-repo` stays a pure rewriter for non-Go repos and for users without a Go toolchain on PATH.",
      "Closes the gap that allowed v4.12.0's bug class to ship in the first place: the rewriter is byte-level and CAN'T detect SEMANTIC desyncs (e.g. a hard-coded sibling literal `\"9\"` next to a `{base}-v9` neighbor that the rewriter bumped to `-v10`). With `--strict`, that class of bug fails the same `gitmap-v24 fix-repo --strict` invocation that would otherwise produce the broken commit, instead of waiting for CI minutes later.",
      ...
      "Constants centralized in `gitmap-v24/constants/constants_fixrepo.go` per the no-magic-strings rule: `FixRepoExitTestsFailed = 9`, `FixRepoFlagStrict = \"strict\"`, plus 7 message format strings (`FixRepoMsgStrictSkipDryRun`, `…NoGoFiles`, `…NoPackages`, `…RunFmt`, `…PassFmt`, `FixRepoErrStrictMissing`, `FixRepoErrStrictFailFmt`). Help system updated: `HelpFRStrict` line added in `constants_fixrepohelp.go`; `HelpFixRepoExitCodes` extended with `9 tests-failed`; `rootusageflags.go::printUsageFixRepoFlags` prints the new line; `gitmap-v24/helptext/fix-repo.md` documents the synopsis, behavior step 6, and the new exit code.",
      "Tests: new `gitmap-v24/cmd/fixrepo_strict_packages_test.go` pins five contracts on the pure derivation helper — empty-input → nil; root + subdirs → correct `.` and `./<dir>` patterns; outside-root paths silently skipped; Windows backslashes normalized to forward slashes (Windows-only branch); deterministic sort across 20 repeat calls (defends against the map-iteration randomization that would otherwise flake CI). The orchestration layer (`runFixRepoStrict`, `invokeGoTest`) is intentionally untested at the unit level because shelling out to `go test` is the integration the strict mode IS — exercised by the next `gitmap-v24 fix-repo --strict` run in CI.",
      "Files: `gitmap-v24/cmd/fixrepo.go` (option field + dispatcher exit-9 branch), `gitmap-v24/cmd/fixrepo_flags.go` (parser + `isFixRepoStrictArg`), `gitmap-v24/cmd/fixrepo_strict.go` (new), `gitmap-v24/cmd/fixrepo_strict_packages.go` (new), `gitmap-v24/cmd/fixrepo_strict_packages_test.go` (new), `gitmap-v24/cmd/rootusageflags.go` (help line wiring), `gitmap-v24/constants/constants_fixrepo.go` (exit code + flag + 7 messages), `gitmap-v24/constants/constants_fixrepohelp.go` (help strings), `gitmap-v24/helptext/fix-repo.md` (synopsis + behavior + exit codes), `fix-repo.ps1` (synopsis + parser + `Invoke-PostRewriteStrict` + main flow), `src/constants/index.ts` (VERSION → v4.14.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.13.0",
    ...
    items: [
      ...
      "Backwards-compat note: any external tool that grepped for the literal `MISMATCH for ` substring in stderr will need to switch to `verify-cmd-faithful:` or the `[FAIL]` tag. The integration goldens in `gitmap-v24/cmd/testdata/clonestream_diag_*.stderr.golden` are unaffected because they exercise the silent-on-match path; no fixture regeneration was required.",
      "Files: `gitmap-v24/cmd/clonetermverify.go` (banner format rewrite + `[FAIL]` prefixing), `gitmap-v24/cmd/clonetermverifytestbanner.go` (new — `PrintCmdFaithfulReportForTest`), `gitmap-v24/cmd/clonetermverify_test.go` (updated assertions + file-header convention note), `gitmap-v24/constants/constants_clone_term.go` (4 new constants), `src/constants/index.ts` (VERSION → v4.13.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.12.0",
    ...
    items: [
      "Closed a long-standing test desync in `gitmap-v24/cmd/replaceversionparse_test.go::TestPairsForTarget` and `gitmap-v24/cmd/replaceaudit_test.go::TestBuildAuditNeedles`. Both tests passed `current=9` (or implied a 1-digit current) into the function under test, but their hard-coded `want` slices included the literal string `\"gitmap-v24\"` next to a `\"gitmap-v24/v9\"` neighbor — an internally contradictory expectation that drifted whenever the project's own version was bumped past v9. After every width-crossing release the same 4-line edit had to be re-applied by hand; if the edit was missed, the test silently locked in the wrong contract.",
      ...
      "Fix: both tests now (1) bind `target` and `current` to named local constants, (2) build the expected `want` slice with `fmt.Sprintf(\"gitmap-v%d\", current)` / `fmt.Sprintf(\"gitmap-v24/v%d\", current)` instead of hard-coded literals, and (3) assert via the derived values. The captured digit and the input version are now structurally the same variable — desync is no longer expressible.",
      ...
      "Verified the production rewrite engine (`gitmap-v24/cmd/fixrepo_rewrite.go::applyAllTargets`) is already width-safe: it folds targets sequentially with a single-pass byte scan and an ASCII-digit lookahead in `writeOneTokenHit`, so a v9-pass on a `-v12` token correctly skips (next byte `2` is a digit) and the v12-pass replaces. No production code change needed — the bug was purely in test expectations.",
      "Files: `gitmap-v24/cmd/replaceversionparse_test.go` (TestPairsForTarget rewritten + new TestPairsForTargetWidthCrossing, `fmt` import added), `gitmap-v24/cmd/replaceaudit_test.go` (TestBuildAuditNeedles rewritten + new TestBuildAuditNeedlesWidthCrossing), `src/constants/index.ts` (VERSION → v4.12.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.11.0",
    ...
    items: [
      "Added a new `fix-repo gofmt audit (affected .go files only)` step to `.github/workflows/ci.yml` (`lint` job), placed BEFORE the existing global `gofmt -l .` hard-gate. It diffs `gitmap-v24/**/*.go` against the PR base (`github.event.pull_request.base.sha`) or `HEAD~1` for pushes, filters changed files to those containing the canonical fix-repo signature `[A-Za-z0-9._-]+-v[0-9]+`, and runs `gofmt -l` on ONLY that subset.",
      "On a hit it emits an annotated GitHub error (`::error title=fix-repo gofmt regression::…`) that names the root cause explicitly — \"the v4.8.0+ post-rewrite gofmt step did not run on them\" — followed by the file list, the full `gofmt -d` diff, and three labeled fix recipes: (1) local `gofmt -w .` + amend, (2) re-run `gitmap-v24 fix-repo --all` (auto-formats since v4.8.0), or (3) re-run `pwsh -File fix-repo.ps1 -All` (auto-formats since v4.9.0). This narrows the failure mode from \"some file isn't gofmt-clean, look around\" to \"a fix-repo bump bypassed the auto-format step, here's how to recover\" — readable in a single scroll.",
  ...
  },
  {
    ...
    items: [
      "New end-to-end test `gitmap-v24/tests/fixrepo_test/TestFixRepoGofmtCleanAfterRewrite` builds the gitmap-v24 binary, materializes a throwaway git repo whose origin URL ends in `repo-v9.git` containing two Go files with column-aligned `map[string]string` and `const (...)` blocks straddling the v8/v9/v10 width boundary, runs `gitmap-v24 fix-repo --all --verbose` against it, and asserts both that the output contains a `gofmt:` summary line (proving the v4.8.0 step ran) AND that `gofmt -l .` is silent afterward (proving alignment is preserved).",
  ...
  },
  {
    ...
    items: [
      "`fix-repo.ps1` now mirrors `gitmap-v24 fix-repo`'s v4.8.0 behavior: after the token sweep, every modified `.go` file is collected and passed in a single `gofmt -w @($GoFiles)` invocation, guarded by `Get-Command gofmt -ErrorAction SilentlyContinue` so non-Go environments degrade gracefully with a stderr WARN instead of a hard failure.",
      ...
      "Why this matters for CI: width-crossing version bumps (e.g. `gitmap-v24` → `gitmap-v24`) silently broke column alignment in Go map literals / const blocks because the byte-level token rewriter doesn't understand Go syntax. The Go binary closed this gap in v4.8.0; the `.ps1` helper kept the gap open whenever someone (or a bootstrap script) invoked the script path. Both engines now produce gofmt-clean output by default, so the `cd gitmap-v24 && gofmt -w .` manual step is no longer needed regardless of which entry point you use.",
  ...
  },
  {
    ...
    items: [
      "`gitmap-v24 fix-repo` (alias `fr`) gains a post-rewrite gofmt pass: after the byte-level token sweep finishes, every modified `.go` file is collected and passed in a single batched `gofmt -w <files...>` invocation. This closes the long-standing gap where width-crossing version bumps (e.g. `gitmap-v24` → `gitmap-v24`) silently broke column alignment in Go map literals / const blocks and tripped CI's `gofmt -l .` gate.",
      ...
      "Implementation: new `gitmap-v24/cmd/fixrepo_gofmt.go` with `runFixRepoGofmt` (orchestrator + dry-run / empty / lookup guards), `invokeGofmt` (batched exec), and `isGoSourceFile` (extension check). `fixRepoSweepResult` (in `gitmap-v24/cmd/fixrepo_scan.go`) gained a `goFiles []string` field that the per-file branch in `processFixRepoFile` appends to whenever a rewrite landed in a `.go` source. `runFixRepo` (in `gitmap-v24/cmd/fixrepo.go`) now calls `runFixRepoGofmt(result.goFiles, opts)` immediately after `emitFixRepoSummary` and before the exit decision.",
      "New constants in `gitmap-v24/constants/constants_fixrepo.go`: `FixRepoMsgGofmtFmt`, `FixRepoMsgGofmtSkip`, `FixRepoMsgGofmtNoneFmt`, `FixRepoErrGofmtFmt`, `FixRepoErrGofmtMissing` — keeps the no-magic-strings rule honored.",
      ...
      "Files: `gitmap-v24/cmd/fixrepo.go`, `gitmap-v24/cmd/fixrepo_scan.go`, `gitmap-v24/cmd/fixrepo_gofmt.go` (new), `gitmap-v24/constants/constants_fixrepo.go`, `gitmap-v24/constants/constants.go` (Version → 4.4.0), `src/constants/index.ts` (VERSION → v4.8.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.7.0",
    ...
    items: [
      "`installer-smoke` job in `.github/workflows/release.yml` was failing post-release with `Error: Binary not found or not executable at $DEST/gitmap-v24` even though `install.sh` had correctly written the binary to `$DEST/gitmap-cli/gitmap-v24` (canonical layout per `gitmap-v24/constants/deploy-manifest.json`).",
      "Root cause: `.github/scripts/smoke-installer.sh#load_deploy_manifest` parsed `appSubdir`, `binaryName.unix`, and `legacyAppSubdirs` out of `deploy-manifest.json` using `awk -F'\"'`. On the GitHub `ubuntu-latest` runner the embedded double-quote field separator combined with `set -euo pipefail` and process substitution intermittently returned the empty string for `APP_SUBDIR`, collapsing the candidate loop to look for `$DEST//gitmap-v24` and `$DEST/gitmap-v24` — both nonexistent — and reporting a phantom `$BIN=$DEST/gitmap-v24` path.",
  ...
  },
  {
    ...
    items: [
      "`gitmap-v24/cmd/clonefixrepo.go`: replaced direct type assertion `runErr.(*exec.ExitError)` with `errors.As(runErr, &exitErr)` so wrapped exec errors still propagate the child exit code through the `clone-fix-repo` / `clone-fix-repo-pub` pipelines. Satisfies golangci-lint `errorlint` and keeps exit-code fidelity for chained steps.",
      "`gitmap-v24/cmd/visibilityapply.go`: collapsed `func parseVisibilityOutput(_ string, out string) string` to `func parseVisibilityOutput(_, out string) string` per gocritic `paramTypeCombine`.",
      "`gitmap-v24/constants/constants_visibility.go`: spelling fix `centralised` → `centralized` per misspell linter.",
      "Files: `gitmap-v24/cmd/clonefixrepo.go`, `gitmap-v24/cmd/visibilityapply.go`, `gitmap-v24/constants/constants_visibility.go`, `src/constants/index.ts` (VERSION → v4.6.0), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.5.0",
    ...
    items: [
      ...
      "Help & UI: top-level `gitmap-v24 help` now lists `clone-fix-repo` and `clone-fix-repo-pub` in the Utilities group; the `--compact` line is updated; web docs (`src/data/commands.ts`) gain full entries with examples and bidirectional `seeAlso` cross-links between `cfr`, `cfrp`, `clone`, `fix-repo`, and `make-public`. Shell completion's generated list (`gitmap-v24/completion/allcommands_generated.go`) picks up `cfr`, `cfrp`, `clone-fix-repo`, and `clone-fix-repo-pub`.",
      "New files: `gitmap-v24/cmd/clonefixrepo.go`, `gitmap-v24/constants/constants_clonefixrepo.go`, `gitmap-v24/helptext/clone-fix-repo.md`, `gitmap-v24/helptext/clone-fix-repo-pub.md`. Edited: `gitmap-v24/cmd/rootcore.go` (dispatch), `gitmap-v24/cmd/rootusageflags.go` (utilities help), `gitmap-v24/constants/constants_helpgroups.go` (compact line), `gitmap-v24/constants/cmd_constants_test.go` (uniqueness test entries), `src/constants/index.ts` (VERSION → v4.5.0), `src/data/commands.ts` (UI entries), `src/data/changelog.ts` (this entry).",
    ],
  },
  {
    version: "v4.4.0",
    ...
    items: [
      "`gitmap-v24 release-pull` (alias `relp`) now accepts three pull modes — `--ff-only` (default, preserves the original safety guarantee), `--rebase` (with auto-abort on conflict so the working tree is never left mid-rebase), and `--merge` (creates a merge commit when fast-forward isn't possible). Modes are mutually exclusive; passing more than one fails fast with a precise error listing the conflicting flags. All other args after the mode flag are forwarded verbatim to `gitmap-v24 release`, including `--dry-run` and `--verbose`.",
      "Top-level `gitmap-v24 help` now renders a dedicated **Fix-repo flags** section listing `-2` (default), `-3`, `-5`, `--all`, `--dry-run`, `--verbose`, `--config`, plus the full exit-code legend (`0 ok | 2 not-a-repo | 3 no-remote | 4 no-vN-suffix | 5 bad-version | 6 bad-flag | 7 write-failed | 8 bad-config`). New constants live in `gitmap-v24/constants/constants_fixrepohelp.go`; rendering is wired through `printUsageFixRepoFlags()` in `gitmap-v24/cmd/rootusageflags.go`.",
      ...
      "Files: `src/constants/index.ts` (VERSION → v4.4.0), `src/data/changelog.ts` (this entry), plus the prior-step changes to `gitmap-v24/cmd/releasepull.go`, `gitmap-v24/helptext/release-pull.md`, `gitmap-v24/cmd/rootusageflags.go`, `gitmap-v24/constants/constants_fixrepohelp.go`, and `src/data/commands.ts`.",
    ],
  },
  {
    version: "v3.119.0",
    date: "2026-04-24",
    subtitle: "`gitmap-v24 inject` / `inj`: register an existing folder with Desktop + VS Code (+ DB)",
    items: [
      "New command for the 'I already have this repo on disk, just plug it into my tooling' workflow — no clone, no scan, just register. Forms: `gitmap-v24 inject` (cwd), `gitmap-v24 inject <folder>` (absolute, relative, or ~-prefixed), `gitmap-v24 inj <folder>` (short alias).",
      ...
      "Files: `gitmap-v24/cmd/inject.go` (new), `gitmap-v24/helptext/inject.md` (new), `gitmap-v24/constants/constants_cli.go` (CmdInject + CmdInjectAlias), `gitmap-v24/constants/constants_v331.go` (Msg/Warn/Err constants), one dispatch entry in `gitmap-v24/cmd/rootcore.go`. Reuses `resolveCloneNextFolder` for path resolution so error messages stay consistent with `cn <folder>`.",
    ],
  },
  {
    version: "v3.118.0",
    date: "2026-04-24",
    subtitle: "`gitmap-v24 clone <url>` cds into the cloned folder",
    items: [
      "Single-URL `gitmap-v24 clone <url>` now writes the cloned folder's absolute path to the shell-handoff sentinel, so the wrapper function chdirs the parent shell into the new repo — same UX as `gitmap-v24 cn` and `gitmap-v24 cd`.",
      ...
      "File: `gitmap-v24/cmd/clone.go` — one `WriteShellHandoff(absPath)` call inside `executeDirectClone`, placed after the DB upsert + Desktop registration but before the VS Code open so the cd happens regardless of whether VS Code is installed.",
    ],
  },
  {
    version: "v3.117.0",
    date: "2026-04-24",
    subtitle: "`gitmap-v24 cn vX <folder>` + `gitmap-v24 cn <folder>` (defaults to v++); hero card UI polish",
    items: [
      ...
      "Files: `gitmap-v24/cmd/clonenextfolderdispatch.go` (new), `clonenextfolderdispatch_test.go` (new — 5 test groups), regex extension in `releaserebase.go`, dispatch wiring in `clonenext.go`, new error/default constants. Spec: `spec/01-app/111-cn-folder-arg.md`. Plan: `08-cn-folder-arg-plan.md`.",
  ...
  },
  {
    ...
    items: [
      ...
      "Three copy-pasteable verification commands confirm the v3.92.0+ rename fix is present: (1) `grep '^const Version = '` against `constants.go` must be 3.92.0 or newer; (2) `grep -nE '^func (fileExists|fileExistsLoose)\\('` against `updatedebugwindows.go` must produce no output (helper moved to `gitmap-v24/fsutil` in v3.113.0); (3) `test -f gitmap-v24/fsutil/exists.go` + import check on both cmd/ files must print both paths.",
  ...
  },
  {
    ...
    items: [
      ...
      "New `gitmap-v24/cmd/updatedebugwindows_source_test.go` parses `updatedebugwindows.go` with `go/parser` and asserts two invariants. `TestUpdateDebugWindowsHasFsutilLooseCall` — the file imports the canonical fsutil path AND contains at least one real call expression of the form `fsutil.FileOrDirExists(...)`. The check uses the AST (selector expression), not a substring scan, so comments mentioning the name do not satisfy the invariant.",
  ...
  },
  {
    version: "v3.113.0",
    date: "2026-04-24",
    subtitle: "Centralize `fileExists` / `fileExistsLoose` / `dirExists` into a shared `gitmap-v24/fsutil` package",
    items: [
      ...
      "New package `gitmap-v24/fsutil` (`gitmap-v24/fsutil/exists.go`) exports three predicates with explicitly documented contracts: `FileExists` (strict file, rejects directories), `FileOrDirExists` (loose existence, accepts directories — replaces the prior `fileExistsLoose`), and `DirExists` (strict directory, rejects files). All three short-circuit on the empty string so callers don't need to guard their inputs.",
      "Contract-pinning tests in `gitmap-v24/fsutil/exists_test.go` exercise every variant against a real tempdir, file, missing path, and the empty string. Collapsing two variants now fails a test before it fails a downstream caller.",
      "`gitmap-v24/cmd/updaterepo.go` — removed local `dirExists` and `fileExists`, calls `fsutil.DirExists` / `fsutil.FileExists` instead. `gitmap-v24/cmd/updatedebugwindows.go` — removed local `fileExistsLoose`, calls `fsutil.FileOrDirExists` instead.",
      "`gitmap-v24/cmd/updatedebugwindows_rename_test.go` — repurposed from the v3.112.0 rename pin into a forward-looking guard. The new `TestFsutilMigrationPinned` asserts the cmd package uses `fsutil.*` and exercises both the loose-empty short-circuit and the strict-dir rejection. A contributor reintroducing a local helper would trigger the redeclaration error this test was created to prevent.",
      "Scope: only the `cmd` package is migrated in this release because that is where the redeclaration risk has materialized. The duplicated helpers in `gitmap-v24/release`, `gitmap-v24/lockfile`, `gitmap-v24/localdirs`, `gitmap-v24/vscodepm`, and `gitmap-v24/detector` are functionally fine (different packages = different namespaces) and can migrate to `fsutil` opportunistically.",
  ...
  },
  {
    ...
    items: [
      "Added typed CLI constants for templates subcommand aliases — `CmdTemplatesDiff` (`\"diff\"`), `CmdTemplatesDiffAlias` (`\"td\"`), `CmdTemplatesInit` (`\"init\"`), `CmdTemplatesInitAlias` (`\"ti\"`) — in `gitmap-v24/constants/constants_templates_cli.go`. The const block opts in via `// gitmap-v24:cmd top-level` so the completion generator picks the alias values up automatically on the next `go generate ./completion/...` run.",
      "Generator-aware skip markers on the full subcommand strings: `CmdTemplatesDiff` and `CmdTemplatesInit` carry `// gitmap-v24:cmd skip` line comments. `\"diff\"` would be a no-op (already a top-level command — folder-tree diff via `gitmap-v24/cmd/diff.go`); `\"init\"` would mislead users into typing `gitmap-v24 init` standalone. Both decisions are now documented in source instead of in a side-channel memory note.",
      "`gitmap-v24/cmd/templatesdiff.go` and `gitmap-v24/cmd/templatescli.go` now alias the local `cmdTemplatesDiff*` / `cmdTemplatesInit*` package identifiers to the shared `constants.CmdTemplates*` values, giving the completion generator one source of truth and preventing string drift between the dispatcher switch and the completion table.",
      "Regenerated `gitmap-v24/completion/allcommands_generated.go` — added `\"td\"` and `\"ti\"` in their sorted positions (between `task`/`temp-release` and `tk`/`tpl` respectively). Run `go generate ./completion/...` locally to confirm byte-equality before tagging.",
  ...
  },
  {
    version: "v3.110.0",
    date: "2026-04-24",
    subtitle: "Plan 05 Phase 2 closeout — `gitmap-v24 templates init` (alias `ti`) shipped end-to-end",
    items: [
      "Verified: `gitmap-v24 templates init <lang> [<lang>...] [--lfs] [--dry-run] [--force]` (alias `ti`) is fully implemented in `gitmap-v24/cmd/templatesinit.go` (297 lines), wired into the dispatcher in `templatescli.go`, covered by 9 unit tests in `gitmap-v24/cmd/templatesinit_test.go`, and documented in `gitmap-v24/helptext/templates-init.md` (133 lines).",
      ...
      "UX deviations from original plan: positional `<lang>...` chosen over `--lang <csv>` (more natural for a scaffolder), and `--cwd` deferred (`cd && gitmap-v24 templates init …` covers it). Both deviations recorded in `.lovable/memory/plans/05-templates-polish-plan.md` Phase 2 notes.",
  ...
  },
  {
    ...
    items: [
      ...
      "Verified: `gitmap-v24/templates/corpus_parity_test.go` already enumerates the new five langs (lines 23-30) and asserts every ignore lang has a matching attributes counterpart (line 97 loop). Any future revert to the asset directory will fail CI before merge — no `corpus_test.go` extension required.",
      "Verified: no lang enum drift. The resolver discovers languages via filesystem walk + filename parsing per Plan 04 design, so adding a new lang requires only dropping the file in. Confirmed zero references to `LangJava` / `LangRuby` / etc. in `gitmap-v24/templates/*.go`. `templates list` picks up the new langs automatically.",
      "Plan 05 status: Phase 0 (spec) ✅, Phase 1 (langs) ✅, Phase 3 (`templates diff`) ✅. Remaining: Phase 2 (`gitmap-v24 templates init` scaffolder), Phase 4 README snippet (deferred until `init` lands), Phase 5 (QA + tag).",
  ...
  },
  {
    version: "v3.108.0",
    date: "2026-04-24",
    subtitle: "`gitmap-v24 templates diff` (alias `td`) — preview template changes without writing",
    items: [
      "Added `gitmap-v24 templates diff` (alias `td`) — pure, write-nothing preview of what `gitmap-v24 add ignore <lang>` / `gitmap-v24 add attributes <lang>` would change. Marker-block aware: only the region between the gitmap-managed `BEGIN`/`END` markers is compared, so edits outside the block never trigger a false positive.",
      "Status enum drives behavior and exit codes (mirrors `diff(1)`): `DiffNoChange` → exit 0, `DiffMissingFile` / `DiffMissingBlock` / `DiffBlockChanged` → exit 1, hard errors (bad lang, unreadable tree) → exit 2. This makes the command pre-commit-hook safe: `gitmap-v24 templates diff --lang go || gitmap-v24 add ignore go`.",
      ...
      "Flags: `--lang <name>` (limit to one language, default = every resolvable lang), `--kind ignore|attributes` (default = both), `--cwd <path>` (run against a different working tree). Five unit tests in `gitmap-v24/templates/diff_test.go` pin all four status branches plus blank-line preservation.",
      ...
      "Bumped `constants.Version` to `3.108.0`. New helptext at `gitmap-v24/helptext/templates-diff.md` with the full exit-code table and pre-commit example.",
    ],
  },
  {
    version: "v3.107.0",
    ...
    items: [
      "Extended `gitmap-v24/render/testdata/pretty/` with three new golden-file cases (`007` headings without subtitles, `008` back-to-back collapsible blocks, `009` blank-line preservation) so the renderer's remaining parser branches are pinned by byte-equality before the next round of helptext additions.",
  ...
  },
  {
    ...
    items: [
      "Fixed: root cause of the recurring `pending task already exists ... \\https:\\github.com\\...` + `fatal: could not create leading directories` failure for `gitmap-v24 clone <url1> <url2>` is that the user's deployed `gitmap.exe` on PATH is older than v3.80.0 and does not contain the multi-URL routing fix. Current source already routes 2+ URLs to `runCloneMulti` via `shouldUseMultiClone(cf)` — but the stale binary still reaches `executeDirectClone(url, folderName=<second URL>, ...)`, builds the impossible Windows path `D:\\...\\https:\\github.com\\...`, and crashes git.",
      "Fixed: `gitmap-v24/cmd/clone.go` `executeDirectClone` now refuses URL-shaped folder names early with an actionable message that names the exact recovery commands: `gitmap-v24 doctor`, `gitmap-v24 update`, `gitmap-v24 pending clear --yes`, and the reminder to open a NEW terminal so PATH refreshes. This shape is impossible in current source, so the guard fires only when a stale binary is in use — and the message tells the user exactly that, instead of letting git fail with `Invalid argument`.",
      "Added: `gitmap-v24/cmd/clone_stale_binary_test.go` pins all three reported PowerShell argv shapes (`url1,url2,url3` comma-glued, `url1 url2 url3` PowerShell-split, comma+space mixed) plus the URL detector and the recovery message contents. Regressions in any of the three cooperating predicates now fail CI before the broken binary can ship.",
      ...
      "Action required: the source is correct, but your PATH binary must be replaced. Run `gitmap-v24 doctor` to confirm the active version, `gitmap-v24 update` to redeploy, open a NEW terminal so PATH refreshes, then `gitmap-v24 pending clear --yes` to drop the orphaned row, and retry the clone — it works space- or comma-separated on PowerShell and bash.",
    ],
  },
  {
    version: "v3.94.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Fixed: `gitmap-v24/cmd/updatecleanup_remove.go` now mirrors `glob_error`, per-attempt `remove_retry`, final `remove_fail`, and `remove_ok` into the durable handoff log and the optional `--debug-windows-json` NDJSON sink, so per-file cleanup failures survive even when the detached child console output does not.",
      "Fixed: `gitmap-v24/cmd/updatecleanup_extra.go` now mirrors the previously ad-hoc special-case cleanup branches too — `drive_root_skip`, `drive_root_remove_fail` / `drive_root_remove_ok`, `swap_glob_error`, and `swap_remove_fail` / `swap_remove_ok` now produce durable events instead of stderr-only lines.",
      ...
      "Added: `gitmap-v24/cmd/updatecleanup_handoff_test.go` covering stable handoff-log formatting plus forwarding of `--debug-windows`, `--debug-windows-json`, `GITMAP_UPDATE_CLEANUP_DELAY_MS`, and `GITMAP_DEBUG_WINDOWS_JSON` into the cleanup child. Targeted `go test ./cmd ...` validation passed.",
    ],
  },
  {
    version: "v3.92.0",
    ...
    items: [
      "Fixed: duplicate `fileExists` declaration in `gitmap-v24/cmd/` blocked `go test ./cmd/...`. Renamed the debug-dump copy in `updatedebugwindows.go` to `fileExistsLoose` (file-or-dir, empty-string-safe) and updated its two callers in `dumpDebugWindowsHandoff`. The original `fileExists` in `updaterepo.go` keeps its strict file-only semantics.",
      "Added: `gitmap-v24/cmd/root_url_shortcut_test.go` pins the bare-URL shortcut against the three exact `Unknown command` invocations users keep reporting — `gitmap-v24 https://a,https://b,https://c` (PowerShell paste), `gitmap-v24 https://a, https://b, https://c` (bash paste, comma-then-space), and `gitmap-v24 https://a, https://b https://c` (mixed separators). Also covers single-URL, leading-flag (`--verbose <url>`), SSH-shorthand (`git@github.com:a/b.git`), and GitLab variants, plus negative cases (known subcommand, folder path, empty argv) so the shortcut never grabs a legitimate command. Three test functions, all sub-second, no external deps.",
      ...
      "Diagnostic note for users still hitting `Unknown command: https://...`: the shortcut has been in source since before v3.91.0, but a stale deployed binary on `PATH` will not have it. Run `gitmap-v24 doctor` to confirm the active binary version, then `gitmap-v24 update` to deploy the current source. The startup version-check banner from v3.90.0 also surfaces this gap on every invocation.",
    ],
  },
  {
    version: "v3.57.0",
    date: "2026-04-23",
    subtitle: "Archive system: `gitmap-v24 zip` + `gitmap-v24 unzip-compact` (alias `uzc`) with compact-folder normalization, multi-source resolution (local / HTTP / git), and `ArchiveHistory` SQLite persistence",
    items: [
      "Added `gitmap-v24 unzip-compact` (alias `uzc`) — extracts a local archive, an HTTP(S) URL, or auto-picks the single archive in the current folder, into a single normalized output folder under `<dest>/<archive-base-name>/`. The compact-extract algorithm (`gitmap-v24/archive/extract.go`) uses the temp-dir-then-move strategy: stream-extract into a sibling `.gitmap-uzc-*` temp dir, walk to find the deepest single-child directory chain (capped at `MaxCompactFlattenLayers=4` per the spec's 2..4 tolerance), then promote that real root into the final folder. Result: `xap.zip → xap/xap/<files>` collapses to `dest/xap/<files>`, `xlt.zip → <files>` (no folder) wraps cleanly into `dest/xlt/<files>`, and `mixed.zip → README + src/` lands as `dest/mixed/{README,src}` (no flatten possible).",
      "Added `gitmap-v24 zip` — accepts N heterogeneous sources on the command line (folders, HTTP(S) archive URLs, git URLs) and writes one archive whose format is derived from the `--out` extension. HTTP sources are downloaded into a temp dir using `aria2c` when available, falling back to `net/http` otherwise; git sources are shallow-cloned (`git clone --depth=1`) into a temp dir; both temp workspaces are removed via `defer cleanupAllSources` regardless of success or failure. Each source root maps to `<basename>/` in the archive so multi-source bundles stay tidy.",
      "Compression knobs: `--best`, `--standard` (also `-s`), and `--fast` are mutually exclusive (the cmd layer rejects more than one with `ErrArchiveBadCompression`). Mode → library level mapping in `gitmap-v24/archive/create.go`: gzip uses `BestCompression`/`DefaultCompression`/`BestSpeed`; bzip2 uses `9`/`6`/`1`; xz and zstd use library defaults; zip uses DEFLATE with selective compression so already-compressed extensions (`.png`, `.zip`, etc.) are stored uncompressed.",
      ...
      "Listing mode: `gitmap-v24 uzc --list <archive>` (also `-l`) prints the entry table to stderr without extracting, capped at 50,000 entries to keep malicious archives from exhausting memory. Auto-detect mode: bare `gitmap-v24 uzc` (no positional args) scans the current folder via `AutoDetectSingleArchive` and proceeds only when EXACTLY one archive is found, erroring with the explicit count otherwise (`current folder has N archives — pass one explicitly`).",
      ...
      "Security: `safeJoin` in `gitmap-v24/archive/extract.go` anchors every in-archive path at the destination root, strips `..`/`.` via `filepath.Clean(\"/\" + name)`, and re-validates that the absolute result still lives inside `destAbs` before any write — preventing the G305 path-traversal class entirely. Symlinks inside archives are rejected on purpose (`writeArchiveFile` short-circuits when `entry.LinkTarget != \"\"`) so a malicious symlink can never escape the destination even if the path-traversal guard is somehow bypassed.",
      "Wired into `gitmap-v24/cmd/roottooling.go` via the existing dispatch table (no new branch in `dispatch()`). Reused `cmd_constants_parity_test.go`'s registry — added `CmdUnzipCompact`, `CmdUnzipCompactAlias`, and `CmdZip` to `topLevelCmds()`. Resolved a name collision: `z` was already taken by `zip-group`, so the archive `zip` command ships without a single-letter alias (users wanting one can `gitmap-v24 alias add`).",
      "New help docs under `gitmap-v24/helptext/`: `unzip-compact.md`, `uzc.md`, and `zip.md`. Each documents the full flag list, format coverage table, normalization rules, and the `ArchiveHistory` row schema so users can reason about the persistence side without grepping the codebase.",
      "Bumped `constants.Version` to `3.57.0`. The cmd layer prints `▶ gitmap-v24 <cmd> v<X.Y.Z>` at the top of every archive run so the version that produced any subsequent output / history row is unambiguous.",
    ],
  },
  {
    version: "v3.56.0",
  ...
  },
  {
    ...
    items: [
      ...
      "Refactored `gitmap-v24/detector/detector.go` to walk each repo exactly once: `walkRepoOnce` now collects every interesting file hit (go.mod, package.json, CMakeLists.txt, meson.build, *.vcxproj, *.sln, *.csproj) AND the set of `.sln` parent directories in a single `filepath.Walk` pass, then `classifyHits` feeds the collected hits through the per-language classifiers in memory. The C# `.sln`-precedence rule (a standalone `.csproj` is suppressed when it lives under a `.sln` directory) still holds because `slnDirs` is fully populated before classification runs.",
      "`gitmap-v24/cmd/scanprojects.go` now runs detection across repos with a bounded worker pool (`detectionWorkerCap = 8`, clamped by `runtime.NumCPU()` and `repoCount`). On a 24-repo project this collapses what used to be 24 sequential walks into 3 batches of 8. Each repo's detector call is independent so the only shared state is the channel buffer.",
      "Expanded `constants.ProjectExcludeDirs` (gitmap-v24/constants/constants_project.go) with `.next`, `.nuxt`, `.svelte-kit`, `.turbo`, `.parcel-cache`, `.angular`, `coverage`, `.nyc_output`, `__pycache__`, `.pytest_cache`, `.mypy_cache`, `.ruff_cache`, `.tox`, `.gradle`, `.idea`, `.vs`, `.vscode-test`, `.terraform`, `.serverless`, `tmp`, `temp`, `logs`, `Pods`, `DerivedData`, `wp-content`, `wp-admin`, `wp-includes`, `uploads`. These are PROJECT-DETECTION exclusions only (the top-level `scanner.ScanDir` walk is unaffected and still uses `cfg.ExcludeDirs`); they prevent the detector from descending into trees that cannot legitimately contain a project manifest like `go.mod` or `package.json`.",
      "New `gitmap-v24/cmd/scanbenchmark.go` captures wall-clock timings for every scan phase and appends a timestamped block to `<scanDir>/.gitmap/output/scan-benchmark.log` with the binary version (`gitmap-v24 scan v<X.Y.Z> @ <RFC3339>`), GOOS/GOARCH/CPU count, and one line per phase: `scan.walk`, `scan.buildRecords`, `scan.writeOutputs`, `scan.saveCache`, `scan.dbUpsertRepos`, `scan.tagScanFolder`, `scan.alignDBIDs`, `scan.detectProjects`, `scan.writeProjectJSON`, `scan.dbUpsertProjects`, `scan.importReleases`, `scan.addToDesktop`, `scan.vscodePMSync`, `TOTAL`. Users reporting slow scans can now attach this log instead of guessing — the offending phase is named explicitly.",
      "`executeScan` in `gitmap-v24/cmd/scan.go` now prints `▶ gitmap-v24 scan v<X.Y.Z> — <absDir>` at the top of every run (suppressed under `--quiet`) so the version that produced any subsequent output / benchmark block is unambiguous. The benchmark log path is echoed at the end (`📊 Benchmark log: ...`) so the file is discoverable without having to know the convention.",
    ],
  },
  {
    version: "v3.28.1",
  ...
  },
  {
    ...
    items: [
      ...
      "`gitmap-v24/scripts/install.sh` `download_asset()` now tracks a `strict=1` flag whenever `${VERSION}` is non-empty and routes every failure path through a new `strict_fail()` helper that prints the canonical message and exits 1. The `.zip` alternate-asset fallback (which previously kicked in when the requested `.tar.gz` wasn't listed in `checksums.txt`) is now SKIPPED in strict mode — the tag's own `checksums.txt` is treated as the source of truth for what assets the user asked for. Same logic mirrored in `gitmap-v24/scripts/install.ps1` `Get-Asset` via a new `Stop-Strict` function. Non-strict installs (no `--version` supplied) keep the existing fallback behavior unchanged so brand-new releases that only ship a `.zip` continue to work.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added `gitmap-v24/templates/corpus_parity_test.go` with three new tests centered on a single source of truth (`requiredLangsBothKinds`): (1) `TestCorpusParityRequiredLangsHaveBothKinds` resolves every (lang × {ignore, attributes}) pair through `Resolve`, asserts `SourceEmbed` (overlay leak guard), and asserts non-empty content. (2) `TestListAdvertisesEveryRequiredLang` cross-checks that `List()` — the function that powers `gitmap-v24 templates list` user-facing output — actually surfaces every required entry, so a corpus addition can never become silently undiscoverable. (3) `TestNewLanguagesPresent` is a focused regression guard for this batch specifically, with a tighter error message that mentions the v3.26.0 commitment so a future revert fails loudly with provenance.",
      "Updated `gitmap-v24/helptext/templates.md` Example 1 output block to reflect the full 11-language × 2-kind matrix (24 rows incl. lfs/common). Kept the `node` row as `user` to preserve the existing pedagogical example of how a forked overlay row reads. Order follows the documented `kindRank` (`ignore` → `attributes` → `lfs`) then alphabetical lang, matching what users actually see when running `gitmap-v24 templates list` — verified by `TestListSortedByKindThenLang` which already enforces that ordering globally.",
      "No code changes required in `cmd/templatesinit.go`, `cmd/templatescli.go`, `templates/list.go`, or `templates/resolver.go`: the `parseRelTemplatePath` + `extFor` + `kindRank` path already handles arbitrary `<lang>` values via string matching against the file extension, so adding new langs is a pure-data corpus extension. `gitmap-v24 templates init java ruby php swift kotlin --lfs` works the moment the new files land, and is now backed by `TestExecuteTemplatesInitWritesIgnoreAndAttributes`-style coverage indirectly through the parity test (every required lang resolves, so init's required-`ignore` precondition is satisfied by construction).",
    ],
  },
  {
    version: "v3.25.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v24 templates init` scaffolds `.gitignore` / `.gitattributes` for chosen languages with `--lfs`, `--dry-run`, and `--force`",
    items: [
      "Added `gitmap-v24/cmd/templatesinit.go` implementing `templates init <lang> [<lang>...] [--lfs] [--dry-run] [--force]` (alias `ti`). For each lang, resolves `ignore/<lang>` (required — hard exit on miss) and `attributes/<lang>` (optional — dim soft-skip notice on miss) via the existing `templates.Resolve` overlay-first lookup, then merges each into `./.gitignore` / `./.gitattributes` via `templates.Merge`. Reuses the same gitmap-managed marker block primitive (`# >>> gitmap-v24:<kind>/<lang> >>>`) as `gitmap-v24 add lfs-install`, so re-runs are byte-stable no-ops and the `templates init --lfs` block is interchangeable with `add lfs-install`'s block (same `lfs/common` tag).",
  ...
  },
  {
    version: "v3.22.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v24 changelog` bullets now share the pretty markdown renderer (cyan `\"quotes\"` consistent with `gitmap-v24 help`)",
    items: [
      "Routed `gitmap-v24 changelog` bullet bodies through the shared `gitmap-v24/render` pretty markdown renderer so the output rules stay in lockstep with the help-text pipeline. Concretely, `gitmap-v24/cmd/changelogwrap.go::renderInlineMarkdown` now applies the cyan double-quote rule (rule 2 of the pretty renderer) on top of the existing `**bold**` and `` `inline code` `` ANSI conversion. Single quotes / apostrophes are intentionally untouched — `user's repo` stays plain text, `\"refactor\"` lights up cyan. Previously bullets ignored quotes entirely, which made `gitmap-v24 cl --latest` and `gitmap-v24 help <cmd>` render the same prose differently — confusing when the changelog literally describes help-text changes.",
      "Exported `render.HighlightQuotes` (was `highlightQuotes`) and added `render.HighlightQuotesANSI` in `gitmap-v24/render/pretty.go` so other CLI surfaces with their own block-level layout can opt into the rule without re-implementing it. Refactored `RenderANSI` to share a `tokenToANSI()` `strings.Replacer` builder with `HighlightQuotesANSI` — single source of truth for the sentinel→ANSI mapping, so adding a new token (e.g. a future green-for-paths span) only updates one function. The existing token sentinels (`TokCyanOpen`/`TokCyanClose`, etc.) keep their public names, so `pretty_test.go` and downstream consumers are unaffected.",
      "Added `gitmap-v24/cmd/changelogwrap_test.go` with three regression guards: (1) `\"double quotes\"` produce `ColorCyan`+`ColorReset` and never leak `[C]` / `[/C]` sentinels into terminal output, (2) apostrophes in `user's repo` do **not** trigger cyan styling (rule-2 single-quote passthrough), (3) the pre-existing `**bold**` / `` `code` `` markers (`ChangelogPrettyBoldOpen`, `ChangelogPrettyCodeOpen`) still appear after the quote pass — proving the wiring order didn't break the established conversion.",
      "Documented the rendering contract in `gitmap-v24/helptext/changelog.md` under a new **Rendering** section: which markdown subset is honored (`**bold**`, `` `code` ``, `\"quotes\"`), the depth-based bullet color scheme (green/cyan/dim at depth 0/1/2+), and the `GITMAP_NO_PRETTY=1` opt-out + non-TTY pipe behavior — same env contract as `helptext/print.go::shouldPretty()`, so users who already disable pretty help also disable pretty changelog with no extra config.",
  ...
  },
  {
    version: "v3.19.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v24 help scan` — four end-to-end workflows for `--config` / `--mode` / `--output`",
    items: [
      "Added an **End-to-End Examples** section to `gitmap-v24/helptext/scan.md` with four complete workflows that string the three most-asked-about flags together: (E2E 1) custom config + JSON output + re-clone on another host via `gitmap-v24 clone <file> --target-dir`, exercising hierarchy preservation across machines; (E2E 2) one scan, two consumers — SSH-mode JSON for CI plus CSV for spreadsheets, with a custom `--output-path`; (E2E 3) terminal preview → author config → re-scan with `--output json`, the recommended onboarding flow when you don't yet know what to exclude; (E2E 4) CLI flag overrides config file, demonstrating `config.MergeWithFlags` precedence without mutating the file on disk.",
      "Each example is copy-pasteable: shell-prompt-free commands, real artifact paths under `.gitmap/output/`, and an **Expected output** block where the result isn't obvious. Cross-references to `gitmap-v24 clone` make the scan→clone round-trip discoverable from the scan page alone — users no longer need to read two helptext files to understand the full pipeline.",
  ...
  },
  {
    version: "v3.18.0",
    date: "2026-04-22",
    subtitle: "Pretty renderer wired into `gitmap-v24 help` — TTY-aware, opt-out via `GITMAP_NO_PRETTY`",
    items: [
      "`gitmap-v24 help <cmd>` and `<cmd> --help` / `-h` now route the embedded markdown through `render.RenderANSI` when stdout is a TTY: redundant fenced blocks collapse to yellow `→` lines, `\"double-quoted\"` strings render in cyan, italic subtitles directly under a heading become muted, and bodies under headings get a 2-space indent. Existing call sites in `gitmap-v24/cmd/helpcheck.go` and `gitmap-v24/cmd/rootutility.go` get the new behavior automatically — no flag needed, no per-command edits.",
      "Pipes and redirects keep the raw markdown bytes unchanged: when stdout is not a character device (e.g. `gitmap-v24 help scan | less`, `gitmap-v24 help scan > scan.md`, CI capture), the file is printed verbatim so editors / pagers / file diffs aren't polluted with ANSI escape sequences. TTY detection uses the same dependency-free `os.Stat() & ModeCharDevice` check that `gitmap-v24/cmd/scanprogress.go` uses for stderr — no `golang.org/x/term` import added.",
      "Added an opt-out env var `GITMAP_NO_PRETTY=1`: when set to any non-empty value, the renderer is bypassed and raw markdown is printed even on a TTY. Lets users who prefer plain output (or who hit terminal-emulator quirks) disable the pipeline without losing other gitmap-v24 coloring (status glyphs, scan progress, changelog renderer all stay on).",
      "Added `helptext.PrintRaw(command)` for callers that want to bypass the pretty pipeline unconditionally (future paginator integration, doc generators, snapshot tests). The default `helptext.Print(command)` keeps the same signature so every existing caller in `gitmap-v24/cmd/` upgrades without modification.",
      "Decision recorded in `.lovable/memory/plans/04-templates-ignore-attributes-plan.md`: `gitmap-v24 changelog` stays on its existing structured renderer (`renderChangelogEntry` in `gitmap-v24/cmd/changelogprint.go`). That renderer operates on `release.ChangelogEntry` structs with depth-aware bullets, ordered-list markers, hanging indent, wrap width, and inline `**bold**` / `` `code` `` ANSI — strictly richer than what generic markdown rendering can produce. Routing it through `render.RenderANSI` would be a regression. The pretty renderer stays scoped to free-form markdown sources (helptext today; future doc commands).",
    ],
  },
  {
    version: "v3.17.0",
    ...
    items: [
      ...
      "Added paired-fixture corpus in `gitmap-v24/render/testdata/pretty/case-NNN-*.{in.md,want.txt}`: case-001 collapses a redundant fence, case-002 highlights two quoted spans, case-003 covers heading + subtitle + indent, case-004 verifies a non-matching fence is preserved (no false-positive collapse), case-005 mixes all rules. Adding a new edge case is now a two-file drop — no test code changes.",
      "Added `gitmap-v24/render/pretty_test.go`: `TestPrettyFixtures` table-loops over every `*.in.md` in the directory and fails when any pair drifts. `TestRenderANSISwapsTokens` guards against sentinel-token leakage in CLI output. `TestUnterminatedQuoteClosedDefensively` ensures a stray `\"` can't bleed an open ANSI sequence into the rest of the terminal session.",
    ],
  },
  {
    version: "v3.16.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v24 templates list` / `templates show` — discover the embedded corpus and your overlay",
    items: [
      "Added `gitmap-v24 templates` subcommand router (alias `tpl`) in `gitmap-v24/cmd/templatescli.go`, wired into `dispatch()` in `root.go` (alongside the previously-missed `dispatchAdd` hook, now also re-confirmed). Subcommands: `list` (alias `tl`) and `show <kind> <lang>` (alias `ts`).",
      "Added `gitmap-v24 templates list` — prints a `KIND / LANG / SOURCE / PATH` table with stable kind ordering (ignore → attributes → lfs) then alphabetical lang. The `SOURCE` column reads `user` when the template comes from `~/.gitmap/templates/` and `embed` when it comes from the binary, making it obvious which entries you've customized.",
      "Added `gitmap-v24 templates show <kind> <lang>` — writes the resolved template (overlay > embed) to stdout *with* its audit-trail header preserved, so `gitmap-v24 templates show ignore go > .gitignore.curated` round-trips cleanly and is diffable against future re-curations.",
      "Added `gitmap-v24/templates/list.go` (+ `dirfs.go` empty-FS fallback): walks the embedded `assets/` first, then overlays user entries from `~/.gitmap/templates/` (silent skip when the dir doesn't exist). The merge map is keyed `<kind>/<lang>` so user files always shadow embeds.",
      "Added `gitmap-v24/constants/constants_templates_cli.go`: `CmdTemplates`/`tpl`, sub-aliases `tl`/`ts`, format strings, error strings, and the user-facing `UsageTemplatesRoot` block.",
      "Added `gitmap-v24/templates/list_test.go`: locks in (1) the embedded corpus shows up with `SourceEmbed`, (2) entries are sorted by kind-rank then lang, (3) a user-overlay file flips `SourceEmbed` → `SourceUser` for the matching `<kind>/<lang>`, and (4) `parseRelTemplatePath` correctly rejects wrong-extension/unknown-kind inputs.",
    ],
  },
  {
    version: "v3.15.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v24 add attributes` + `gitmap-v24 add lfs-install` — completes the templates trio",
    items: [
      "Added `gitmap-v24 add attributes [langs...]` (alias `aa`) in `gitmap-v24/cmd/addattributes.go` — mirror of `add ignore` but writes to `./.gitattributes` using the dedicated `# >>> gitmap-attributes (do not edit between markers) >>>` / `# <<< gitmap-attributes <<<` markers. Same single-pass dedupe, atomic write, and `Changed=false` no-op on re-run.",
      "Added `gitmap-v24 add lfs-install` (alias `alfs`) — runs `git lfs install --local` (streaming stdout/stderr so users see the LFS hook installation) then merges the curated `lfs/common.gitattributes` into `./.gitattributes`. The LFS template uses the attributes markers so it co-exists cleanly with `add attributes`.",
      ...
      "Added `gitmap-v24/templates/merge_attributes_test.go`: verifies attributes markers (and that they don't leak the ignore markers), LFS merge writes to `.gitattributes` via attribute markers, attributes idempotence, and the kind→extension mapping (`ignore`→`.gitignore`, `attributes`+`lfs`→`.gitattributes`). The LFS test also locks in `*.svg filter=lfs` MUST NOT appear.",
    ],
  },
  {
    version: "v3.14.0",
    date: "2026-04-22",
    subtitle: "`gitmap-v24 add ignore` — curated, idempotent .gitignore templates with marker-block merge",
    items: [
      "Added `gitmap-v24 add` subcommand router (`gitmap-v24/cmd/rootadd.go`, `gitmap-v24/constants/constants_add.go`) wired into `dispatch()` in `root.go`. Top-level alias `ad`. Routes to `add ignore` (alias `ai`), `add attributes` (alias `aa`, Phase 3), and `add lfs-install` (alias `alfs`, Phase 3).",
      "Added `gitmap-v24 add ignore [langs...]` — composes `common` plus each requested language template into a marker-block (`# >>> gitmap-ignore ... >>>` / `# <<< gitmap-ignore <<<`), preserves user-authored entries below `# user entries`, and writes atomically via temp file + rename.",
      "Added `gitmap-v24/templates/merge.go` engine: single-pass dedupe keyed on trimmed line (comments dedupe too), audit-trail headers (`# source:` / `# kind:` / `# lang:` / `# version:`) stripped before merge, blank-line collapse, idempotent — re-running with identical args yields a byte-identical file (`MergeResult.Changed=false`).",
      "Seeded curated corpus in `gitmap-v24/templates/assets/` (Phase 1): `ignore/{common,go,node,python,rust,csharp}.gitignore`, `attributes/{common,go,node,python,rust,csharp}.gitattributes`, and `lfs/common.gitattributes`. Common attributes normalize `*.svg` to `text eol=lf` (NOT LFS — SVG is XML and diffs cleanly); `*.bat`/`*.cmd`/`*.ps1` keep CRLF; raster/archive formats forced binary.",
      "Added `gitmap-v24/templates/{embed.go,resolver.go,materialize.go,paths.go}` (Phase 0): `//go:embed all:assets`, `~/.gitmap/templates/` user overlay (overlay > embed resolution order) so users on read-only install paths (e.g. `C:\\Program Files\\gitmap-v24`) can still customize, idempotent first-run materializer that skips any already-present file.",
  ...
  },
  {
    ...
    items: [
      "Fixed `gitmap-v24 update-cleanup` Access-Denied errors on Windows for `gitmap-update-<pid>.exe` and `gitmap.exe.old`. Root cause: cleanup was invoked from inside the running handoff process, which still held an exclusive file lock on its own binary. New Phase 3 in `gitmap-v24/cmd/updatehandoff_phase3.go` resolves the freshly-deployed `gitmap.exe` via `exec.LookPath`, then spawns `cmd.exe /C ping 127.0.0.1 -n 3 >nul & start \"\" /B \"<deployed>\" update-cleanup` detached. The 2s ping delay lets the handoff process exit and release its lock; the deployed binary (a different file) then removes both leftovers cleanly.",
      "Removed obsolete `scheduleHandoffSelfDelete` from `gitmap-v24/cmd/update.go` — superseded by the Phase 3 cleanup handoff which handles the handoff copy AND the `.exe.old` backup in one pass.",
  ...
  },
  {
    ...
    date: "2026-04-20",
    items: [
      "Added pinned-version install snippet auto-appended to every GitHub release body (`gitmap-v24/release/installsnippet.go`), with PowerShell + bash one-liners that hard-code the published tag. Idempotent via `<!-- gitmap-pinned-install-snippet:<tag> -->` marker.",
      ...
      "Renamed `gitmap-v24` → `gitmap-v24` across Go constants, install/uninstall scripts, spec docs, helptext, README, React data files, and GitHub workflows. Only `.gitmap/` artifacts retain v3 references (immutable per project policy).",
  ...
  },
  {
    ...
    date: "2026-04-19",
    items: [
      "Fixed v15 Phase 1.4 migration crash on databases first created at v3.5.0+: `GoProjectMetadata` and `PendingTask` rebuilds threw `no such column: Id` because both tables were already singular before v15, so the canonical `CREATE TABLE IF NOT EXISTS` pass had already produced the v15-shaped table (with `{Table}Id` PK). New `adaptOldColumnList()` in `gitmap-v24/store/migrate_v15rebuild.go` detects the existing PK column via `columnExists()` and rewrites the leading `Id` token in `OldColumnList` to `{Table}Id` when needed — idempotent, no-op for genuine legacy paths.",
      "Fixed `go vet` `non-constant format string` warning in `gitmap-v24/movemerge/finalize.go:50` by reshaping `logErr(prefix, msg string)` to accept a pre-formatted message and moving `fmt.Sprintf(constants.ErrMMPushFailFmt, sha)` to the call site.",
      "Fixed `gitmap-v24/store/migrations.go` build error caused by an orphan `\"github.com/alimtvnetwork/gitmap-v24/gitmap-v24/constants\"` import.",
      ...
      "Fixed `cd` / `go` constant collision — removed duplicate `CmdCDCmd` / `CmdCDCmdAlias` from `constants_cli.go` and repointed `gitmap-v24/cmd/rootdata.go` dispatch at the canonical `CmdCD` / `CmdCDAlias` constants in `constants_cd.go`.",
      "Added `gitmap-v24/cmd/cmdconstants_unique_test.go` (+ helpers): parses every `constants_*.go` using the same `gitmap-v24:cmd top-level` / `gitmap-v24:cmd skip` markers as the completion generator and fails the suite when two distinct `Cmd*` identifiers claim the same string value. Catches redeclarations and dispatch shadowing at CI time before the build phase.",
      "Added parallel `gitmap-v24 pull` worker pool (`pullparallel.go`) with a mutex-guarded `BatchProgress` tracker, opt-in via `--parallel <N>`.",
      "Added `--only-available` pull pre-filter (`pullfilter.go`) that intersects the target repo list with `FindNext` results so `gitmap-v24 pull --only-available` skips repos with no new tags. Fail-open if the database is inaccessible.",
      "Added `gitmap-v24 help probe` and `gitmap-v24 help sf` docs (synopsis, flags, examples, 3–8 line realistic terminal simulation) in `gitmap-v24/helptext/`.",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "`gitmap-v24 update-cleanup` now scans the active PATH directory, the PATH-derived deploy directory, the configured deploy directory, and the repo build output directory so stale `.old` backups are removed even when `powershell.json` points to an older location.",
      "`gitmap-v24 update-cleanup` now removes leftover `gitmap-update-*` artifacts from deploy/build locations in addition to `%TEMP%`, preventing handoff files from being left behind after update flows that switch between deploy targets.",
    ],
  },
  {
    version: "v2.82.0",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "Hidden `set-source-repo` command — persists source repo path to DB so `gitmap-v24 update` always uses the correct location after repo moves.",
      ...
      "`go-winres` CI failure fixed — moved `winres.json` to `gitmap-v24/winres/` where `go-winres make` expects it.",
  ...
  },
  {
    ...
    items: [
      ...
      "Installer banner now displays version number (`gitmap-v24 installer v1.0.0`).",
      ...
      "Standardized lowercase \"gitmap-v24\" branding across all installer output.",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "New `gitmap-v24 version-history` (`vh`) command: displays all version transitions for the current repo with `--limit N` and `--json` support.",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "`gitmap-v24 clone-next` now flattens by default: clones into the base name folder (no version suffix). `gitmap-v24 cn v++` inside `macro-ahk-v15` clones into `macro-ahk/`.",
      "`gitmap-v24 clone <url>` auto-flattens versioned URLs when no custom folder is given (e.g., `wp-onboarding-v13` clones into `wp-onboarding/`).",
  ...
  },
  {
    ...
    date: "2026-04-16",
    items: [
      "`gitmap-v24 doctor setup` checks: verifies `git-setup.json` resolution from the installed binary location and warns when the shell wrapper is not loaded.",
      "Shell wrapper scripts now export `GITMAP_WRAPPER=1` to detect raw binary vs. wrapper invocation at runtime.",
      "`gitmap-v24 setup` resolves `git-setup.json` relative to the binary path instead of CWD, fixing 'file not found' errors when run from other directories.",
      "Post-setup verification step checks whether the shell function is loaded and prints reload instructions if not.",
      "`gitmap-v24 cd` prints a stderr warning when called without the shell wrapper, guiding users to run `gitmap-v24 setup` or reload their profile.",
    ],
  },
  {
    version: "v2.72.0",
  ...
  },
  {
    ...
    items: [
      ...
      "`gitmap-v24 update` PATH sync now includes full 3-step fallback: direct copy, rename-then-copy, and kill stale processes.",
  ...
  },
  {
    ...
    date: "2026-04-08",
    items: [
      "`run.ps1` and `run.sh` now auto-detect the globally installed `gitmap-v24` location and deploy there instead of using a hardcoded path.",
      ...
      "Replaced hardcoded `E:\\bin-run` in `gitmap-v24 doctor` fix suggestion with dynamic guidance.",
    ],
  },
  {
    version: "v2.66.0",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Added `gitmap-v24 install scripts` — clones gitmap-v24 scripts to a local folder (Windows: D:\\ drive, Linux: ~/Desktop/gitmap-scripts).",
    ],
  },
  {
    version: "v2.63.0",
    date: "2026-04-07",
    items: [
      "Added `installed-dir` (alias `id`) command — prints the active gitmap-v24 binary path and directory.",
      ...
      "Fixed `gitmap-v24 install` on Ubuntu: runs `apt-get update` before install; added `-y` flag and error logging to `.gitmap/logs/`.",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Auto-detect pending release branch — running `gitmap-v24 r` on a `release/*` branch with no tag now completes the release instead of erroring.",
      "Running `gitmap-v24 release v1.1.0` while on `release/v1.1.0` delegates to the pending release flow automatically.",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Post-release install hints — after releasing a gitmap-v24 repo, the CLI prints Windows (PowerShell) and Unix (Bash) install one-liners.",
      "Repo detection via `GitmapRepoPrefix` constant — only gitmap-v24 repos trigger the install hint output.",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Fixed `-y` / `--yes` flag silently ignored when placed after the version argument (e.g., `gitmap-v24 release v2.55 -y`).",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "Clone-on-missing-path for `gitmap-v24 update` — when the user provides a non-existent path at the interactive prompt, the system clones the source repo there instead of rejecting it.",
  ...
  },
  {
    ...
    items: [
      ...
      "Auto-confirm skips the commit prompt — enables `gitmap-v24 release v2.55.0 -y` without user input.",
  ...
  },
  {
    ...
    date: "2026-04-07",
    items: [
      "`gitmap-v24 update` now validates the saved source repo path exists on disk before using it; falls back to SQLite DB.",
      ...
      "Release pipeline `dist` directory fix — explicit `working-directory: gitmap-v24/dist` prevents `cd: dist` failure in CI.",
  ...
  },
  {
    version: "v2.53.0",
    items: [
      "New `gitmap-v24 help-dashboard` (alias `hd`) command to serve the documentation site locally.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added `gitmap-v24 install` system with SQLite-backed `InstalledTools` table tracking name, version, manager, and timestamps.",
      "Added `gitmap-v24 uninstall` command with `--dry-run`, `--force`, and `--purge` flags.",
      ...
      "Reorganized `gitmap-v24 help` with `--compact` flag and 17 categorized command groups.",
      "Repository renamed from `git-repo-navigator` to `gitmap-v24`.",
    ],
  },
  {
    version: "v2.51.0",
    items: [
      "Reorganized `gitmap-v24 help` output into 17 categorized command groups (Scanning, Cloning, Git Operations, Navigation, Release, etc.).",
      "Added `--compact` flag to `gitmap-v24 help` for a minimal command-and-alias-only listing.",
      "`gitmap-v24 help --compact <group>` filters compact output by group name (case-insensitive, falls back to all on no match).",
  ...
  },
  {
    ...
    items: [
      ...
      "`gitmap-v24 ls` now prints resolved DB path when `--verbose` or zero repos found.",
  ...
  },
  {
    ...
    items: [
      ...
      "Added interactive TUI mode (`gitmap-v24 interactive` / `gitmap-v24 i`) built with Bubble Tea.",
  ...
  },
  {
    ...
    items: [
      ...
      "Improved `gitmap-v24 ls <type>` output with labeled fields and inline `cd` examples.",
      "Added shell completion for `release`, `release-branch`, `group`, `multi-group`, and `list` commands.",
      "Fixed duplicate hints after `gitmap-v24 ls <type>` output.",
    ],
  },
  {
    version: "v2.13.0",
    items: [
      "Added group activation: `gitmap-v24 g <name>` sets a persistent active group for batch pull/status/exec.",
      "Added `multi-group` (mg) command for selecting and operating on multiple groups at once.",
      "Added `gitmap-v24 ls <type>` filtering: `gitmap-v24 ls go`, `gitmap-v24 ls node`, `gitmap-v24 ls groups`.",
      ...
      "`gitmap-v24 list-versions` now shows changelog notes.",
    ],
  },
  {
    version: "v2.12.0",
    date: "2026-03-14",
    items: [
      "Added `gitmap-v24 list-versions` (`lv`) command: show all release tags sorted highest-first with changelog.",
      "Added `gitmap-v24 revert <version>` command: checkout tag and handoff rebuild.",
  ...
  },
  {
    ...
    date: "2026-03-06",
    items: [
      "Added `gitmap-v24 cd` (`go`) command: jump to any tracked repo by slug or partial name.",
      "Added `gitmap-v24 watch` (`w`) command: live terminal dashboard monitoring repo status.",
      "Added `gitmap-v24 diff-profiles` (`dp`) command: compare two profiles side-by-side.",
      "Added clone progress bars with retry logic and Windows long-path warnings.",
      "Added `gitmap-v24 gomod` (`gm`) command: rename Go module path across an entire repo.",
      "Added `gitmap-v24/Makefile` as a thin wrapper around `run.sh` for Linux/macOS.",
  ...
  },
  {
    ...
    date: "2026-03-06",
    items: [
      "Added `gitmap-v24 profile` (`pf`) command: manage multiple database profiles.",
      "Added `gitmap-v24 export` (`ex`) command: export the full database as portable JSON.",
      "Added `gitmap-v24 import` (`im`) command: restore database from a backup file.",
      "Added `gitmap-v24 bookmark` (`bk`) command: save and replay command+flag combinations.",
      "Added `gitmap-v24 stats` (`ss`) command: aggregated usage statistics from command history.",
      "Added `gitmap-v24 history` (`hi`) command: queryable audit trail of CLI executions.",
      "Added `gitmap-v24 amend` (`am`) command: rewrite author name/email on existing commits.",
      "Added `gitmap-v24 seo-write` (`sw`) command: automated SEO commit scheduler.",
    ],
  },
  {
    version: "v2.4.0",
    date: "2026-03-06",
    items: [
      "Added `gitmap-v24 latest-branch` (`lb`) command.",
      "Positional integer shorthand: `gitmap-v24 lb 3` equals `gitmap-v24 lb --top 3`.",
  ...
  },
  {
    ...
    date: "2026-03-06",
    items: [
      "Added `gitmap-v24 list-releases` (`lr`) command.",
      "Enhanced `gitmap-v24 scan` to import `.gitmap/release/v*.json` metadata files.",
