...
const REPO_RAW = "https://raw.githubusercontent.com/alimtvnetwork/gitmap-v25/main";

const installCmds = {
  windows: `irm ${REPO_RAW}/gitmap-v25/scripts/install.ps1 | iex`,
  unix: `curl -fsSL ${REPO_RAW}/gitmap-v25/scripts/install.sh | sh`,
};
...
const installPinnedCmds = {
  windows: `$ver = 'v3.99.0'
$installer = irm ${REPO_RAW}/gitmap-v25/scripts/install.ps1
& ([scriptblock]::Create($installer)) -Version $ver -NoDiscovery`,
  unix: `curl -fsSL ${REPO_RAW}/gitmap-v25/scripts/install.sh \
    ...
    <DocsLayout>
      <div className="max-w-4xl">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-heading font-bold docs-h1">Install &amp; Update gitmap-v25</h1>
        </div>
        ...
        <section className="mb-12">
          ...
          <Card className="p-4 mb-4">
            ...
            <CodeBlock code={`gitmap-v25 update`} title="Any platform" language="bash" />
          </Card>
          <Card className="p-4">
            <h3 className="text-sm font-semibold mb-2 docs-h3">Pin to a specific version</h3>
            <CodeBlock code={`gitmap-v25 self-install --version v3.99.0 --yes`} title="Any platform" language="bash" />
        ...
        <section className="mb-12">
          ...
          />
          <CodeBlock
            code={`gitmap-v25 version                   # confirms binary on PATH + build info
gitmap-v25 doctor                    # PATH, profile snippets, deploy folder, DB
gitmap-v25 setup print-path-snippet  # prints the exact bytes the installer wrote`}
            title="Verification"
            language="bash"
          />
        </section>
        ...
        <section className="mb-12">
          <SectionHeader
            icon={Trash2}
            title="Uninstall"
            kicker="First tries `gitmap-v25 self-uninstall`; falls back to a manual sweep if gitmap-v25 is no longer on PATH."
          />
            ...
            <table className="w-full text-sm border border-border rounded-lg overflow-hidden">
              ...
              <tbody className="divide-y divide-border">
                {[
                  ...
                  ["--keep-data", "Always keep %APPDATA%\\gitmap-v25 (Windows) / ~/.config/gitmap-v25 (Unix)"],
        ...
        <section className="mb-12">
          ...
          <p className="text-xs text-muted-foreground mt-3">
            ...
            <code className="docs-inline-code">gitmap-v25/constants/constants_pathsnippet.go</code> so install.sh,
            install.ps1, and{" "}
            <code className="docs-inline-code">gitmap-v25 setup print-path-snippet</code> emit byte-identical output.
          </p>
        </section>
      </div>
    </DocsLayout>
