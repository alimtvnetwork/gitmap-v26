...
## v5.47.0 — (2026-05-22) — pwsh-runner stdout capture: real-fix via temp-file fd inheritance + per-entry `Icon` for install ctx

- **Long-term fix for Windows CI subprocess capture** (replaces the v5.46.2/v5.46.3/v5.46.4 skips). Root cause: when Go's `os/exec` is given a `bytes.Buffer` for `cmd.Stdout`/`Stderr`, it internally creates an `os.Pipe()` and copies bytes via a goroutine. Under the GitHub Actions `windows-latest` runner's `pwsh -command ". '{0}'"` host, the parent end of that pipe reads EOF immediately even though the child writes correctly — a long-documented pwsh-on-runner stdio bug (see [actions/runner#382](https://github.com/actions/runner/issues/382) and the StepCodex pwsh-stdio case study). **Fix:** `runGitmap` now writes child stdout/stderr to a real OS temp file (true fd inheritance, no Go-pipe goroutine in between) and reads it back after `cmd.Run()`. Works uniformly on Linux/macOS/Windows. The previously-skipped tests (`TestCLI_FailureContext_Scan`, `TestCLI_FailureContext_CloneFromMissingManifest`, `TestCLI_FailureContext_CloneNowMissingManifest`, `TestCloneNowCLI_UserCanceledNonTTY`, `TestScanCLI_ExitCodes/failure_missing_dir`, `TestFixRepoGofmtCleanAfterRewrite`) now run on Windows again.
- **`install ctx` — per-entry `Icon` field** (Windows). `ctxEntry` gains an optional `Icon` string; when set, `categoryCommands` and `leafCommands` emit `reg add <key> /v Icon /d <path> /f` alongside the existing `/ve` write. Applies to both leaves and category cascades. Empty `Icon` (the default) preserves the v5.46.x behavior. Sets up the upcoming icon-glyph pass for the gitmap right-click menu without breaking any current entry.
- **`install ctx` — root-menu de-dupe**: removed the residual duplicate registrations of `90_terminal` and `91_docs` in `ctxMenu()` (the v5.46.1 fix had only deduped the *registration* path; the entry list itself still listed them twice). The duplicates were harmless on Windows 11 but caused icon/extended attribute drift on older Windows 10 builds.
- **Tag-mirroring status update**: persistence (migration 006 columns + `RecordTagReplay` / `LookupTagReplay` / `ClassifyVersionTag`) is complete and unit-tested. Stage-14 orchestrator wiring (walk source tags per commit → `git tag` on dest → optional `release/<tag>` branch → record) is the next milestone and tracked separately under `spec/03-commit-in/08-tag-mirroring-and-release-branches.md`.
...
- **`/install.ps1`** and **`/install.sh`** now live at the repository root. The one-liner installer URL is now:
  - Windows: `irm https://raw.githubusercontent.com/alimtvnetwork/gitmap-v23/main/install.ps1 | iex`
  - macOS / Linux: `curl -fsSL https://raw.githubusercontent.com/alimtvnetwork/gitmap-v23/main/install.sh | sh`
...
- **`release-version-vX.Y.Z.{ps1,sh}` snapshot assets** are no longer attached to non-gitmap releases. Those snapshots hard-code `REPO="alimtvnetwork/gitmap-v23"` and `BINARY_NAME="gitmap"` — uploading them to `img-pdf-v2`, `some-tool-v3`, etc. would mislead users into downloading the gitmap binary from the wrong release page. `pushAndFinalize` in `gitmap/release/workflowfinalize.go` now wraps `buildReleaseVersionSnapshots` in the same `ShouldPrintInstallHint` gate.
...
- `gitmap clone-fix-repo` / `cfrp` now decide whether to run `fix-repo --all`
  ...
  folder. Cloning `gitmap-v23` into `gitmap/` now correctly runs the rewrite
...
- Re-pinned the root `README.md` "Pinned version" section, one-line installers, and version matrix asset URLs to `v4.44.0` / `gitmap-v23.44.0-*`
...
- Re-pinned the root `README.md` "Pinned version" section, one-line installers, and version matrix asset URLs to `v4.42.0` / `gitmap-v23.42.0-*`
...
- Re-pinned the root `README.md` "Pinned version" section, one-line installers, and version matrix asset URLs to `v4.41.0` / `gitmap-v23.41.0-*`
...
- Re-pinned the root `README.md` "Pinned version" section, one-line installers,
  ...
  the new `v4.40.0` release tag and `gitmap-v23.40.0-*` asset names.
...
- `gitmap/cmd/fixrepo_rewrite_scan_test.go`: `TestScannerMatchesRewriter` now
  ...
  instead of hard-coding `"gitmap-v23"` while passing `current = 12` to
  `applyAllTargets`. The hard-coded literal silently disagreed with the
  rewriter's actual output (`gitmap-v23`) and failed CI on every run.
...
- Legacy-ref test data migrated to the current `gitmap-v23` repo namespace.
...
- New `gitmap/cmd/updatedebugwindows_source_test.go` parses
  ...
  - **`TestUpdateDebugWindowsHasFsutilLooseCall`** — the file imports
    `github.com/alimtvnetwork/gitmap-v23/gitmap/fsutil` AND contains at
...
- Installer scripts (`install.ps1`, `install.sh`, `uninstall.ps1`) embedded into the binary via `go:embed` (`gitmap/scripts/embed.go`), with HTTP fallback to `raw.githubusercontent.com/alimtvnetwork/gitmap-v23/main/gitmap/scripts/install.{ps1,sh}` if the embedded copy is missing.
...
- **README.md Go Report Card badge** now points at `github.com/alimtvnetwork/gitmap-v23/gitmap` (the real Go module path set in v3.27.0) instead of the repo root `github.com/alimtvnetwork/gitmap-v23`. The previous URL returned a 404 from the Go module proxy because there is no `go.mod` at the repo root — the module lives one directory down in `gitmap/`. Both the badge image and the click-through report link were updated.
...
- **Go Report Card now resolves the module instead of failing with `could not get latest module version from https://proxy.golang.org/github.com/user/gitmap/@latest`.** The card at https://goreportcard.com/report/github.com/alimtvnetwork/gitmap-v23/gitmap will start scoring the project for the first time after this version is pushed.
...
If left unfixed, anyone running `go install github.com/alimtvnetwork/gitmap-v23/gitmap@latest` would get a `module declares its path as: github.com/user/gitmap but was required as ...` error, and the proxy would refuse to serve the module to downstream tooling.

### Fix

Renamed `github.com/user/gitmap` → `github.com/alimtvnetwork/gitmap-v23/gitmap` across **404 files** in a single atomic sed pass. Also implicitly renamed the sister module `github.com/user/gitmap-updater` → `github.com/alimtvnetwork/gitmap-v23/gitmap-updater`, which lives at the same GitHub path.

Verified post-rename:
- `gitmap/go.mod` now reads `module github.com/alimtvnetwork/gitmap-v23/gitmap`.
- `gitmap-updater/go.mod` now reads `module github.com/alimtvnetwork/gitmap-v23/gitmap-updater`.
...
2. Once the new tag (`v3.27.0`) is pushed, visit https://goreportcard.com/report/github.com/alimtvnetwork/gitmap-v23/gitmap — first visit will trigger a fresh scan against the new module path.
...
## v3.16.0 — (2026-04-20) — uninstall-quick.ps1 multi-binary fix + repo rename to gitmap-v23
...
- **All `gitmap-v23` references → `gitmap-v23`** across the entire repo (45 files, 567 occurrences). Includes install/uninstall one-liners, Go installer constants, helptext, spec docs, post-mortems, the React landing page, and `.lovable/memory/**`.
- **Preserved**: release-asset filenames like `gitmap-v23.49.1-windows-amd64.zip` (where `v4.49.1` is the package version, not the repo name) — only the GitHub URL repo segment changed.

### Why

The uninstall failure was reported by a user who had run gitmap since the v2.x drive-root shim era — their stale `E:\gitmap\gitmap.exe` was never removed and the new `gitmap-cli/` install put a second binary on PATH. The `gitmap-v23` repo rename had been pending since the v3.x line started; bundling both keeps the CHANGELOG narrative simple.
...
- **Remote installer URLs** — Updated `constants_selfinstall.go` `SelfInstallRemotePwsh` and `SelfInstallRemoteBash` from `gitmap-v23` to `gitmap-v23`.
...
## v3.12.0 — (2026-04-20) — Pinned-version release snippet + gitmap-v23 rename
...
- **Unused-import build break** in `gitmap/store/migrations.go` — removed orphaned `"github.com/alimtvnetwork/gitmap-v23/gitmap/constants"` import left over from a prior refactor.
...
- After a successful release, if the repo's remote origin matches the gitmap source repository prefix (`github.com/alimtvnetwork/gitmap-v23`), the CLI now prints install one-liner commands for both Windows (PowerShell) and Linux/macOS (Bash).
...
- Repository renamed from `git-repo-navigator` to `gitmap-v23`; all URLs, scripts, and references updated.
...
- CI release binaries now include version in filenames (e.g., `gitmap-v23.49.1-windows-amd64.zip`).
